...
async function observeAgents(client: RamblaClient, ws: FakeWebSocket): Promise<string> {
...
test("canceled timeline handles and captured state are collectible while their API stays alive", async () => {
  const source = `
    ...
    import { createRamblaApi } from ${JSON.stringify(new URL("./index.ts", import.meta.url).href)};
    ...
    async function batch(mode) {
      ...
      const api = createRamblaApi(driver, { signal: scope.signal });
  ...
  const env = Object.fromEntries(
    Object.entries(process.env).filter(
      ([key]) => !/^(RAMBLA_|EXPO_|E2E_|AGENT_BROWSER_)/.test(key),
    ),
  );
