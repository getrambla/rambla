...
export const Route = createFileRoute("/changelog")({
  head: () =>
    pageMeta(
      "Changelog - Rambla",
      "Product updates, bug fixes, and improvements shipped in each Rambla release. Track new agent providers, mobile features, and daemon changes over time.",
      "/changelog",
    ),
  component: Changelog,
});
...
function HeadingAnchor({ version }: { version: string }) {
  return (
    <a
      ...
      aria-label={`Link to Rambla ${version}`}
    >
    ...
    <article className="changelog-release">
      <div id={releaseAnchor(group.version)} className="changelog-release-heading">
        ...
        <h2 className="changelog-release-title">
          Rambla <span>{group.version}</span>
        </h2>
      </div>
