...
async function viewPhones(page: Page) {
  await page.getByRole("img", { name: "Rambla agent chat", exact: true }).scrollIntoViewIfNeeded();
}
...
test("the three phone mockups show their screen content", async ({ page }) => {
  ...
  await expectScreenContentInsidePhone(page, "Rambla workspace drawer", "18:54");
  await expectScreenContentInsidePhone(page, "Rambla agent chat", "18:53");
  await expectScreenContentInsidePhone(page, "Rambla diff view", "18:55");
});
