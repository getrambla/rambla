...
describe("Codex terminal agent hooks", () => {
  it("installs POSIX and Windows hook commands idempotently", () => {
    ...
    for (const event of codexAgentHookProvider.events) {
      expect(commandHooks(config, event.event)).toEqual([
        {
          command: `if [ -n "$RAMBLA_TERMINAL_ID" ]; then "\${RAMBLA_HOOK_CLI:-paseo}" hooks codex ${event.event}; fi`,
          commandWindows: `if defined RAMBLA_TERMINAL_ID (if defined RAMBLA_HOOK_CLI ("%RAMBLA_HOOK_CLI%" hooks codex ${event.event}) else (paseo hooks codex ${event.event})) else (exit /b 0)`,
        },
      ]);
    }
  ...
  it("preserves unrelated user hooks", () => {
    ...
    expect(stopCommands).toEqual([
      "say codex done",
      'if [ -n "$RAMBLA_TERMINAL_ID" ]; then "${RAMBLA_HOOK_CLI:-paseo}" hooks codex Stop; fi',
    ]);
  });
