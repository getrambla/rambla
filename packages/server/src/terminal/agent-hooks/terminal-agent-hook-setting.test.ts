...
function hookPaths(root: string) {
  return {
    ...
    opencode: join(root, "opencode", "plugins", "rambla-terminal-activity.js"),
  };
}

function createStore(ramblaHome: string, enableTerminalAgentHooks: boolean): DaemonConfigStore {
...
describe("applyTerminalAgentHookSetting", () => {
  it("leaves agent configs untouched when the setting is disabled", () => {
    const root = createTempDir("rambla-hook-setting-");
    const store = createStore(createTempDir("rambla-hook-setting-home-"), false);
  ...
  it("installs agent hooks when the setting is enabled", () => {
    const root = createTempDir("rambla-hook-setting-");
    const store = createStore(createTempDir("rambla-hook-setting-home-"), true);
