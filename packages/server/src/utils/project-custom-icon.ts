...
export async function setProjectCustomIcon(input: {
  ...
  return serialize(input.projectId, async () => {
    ...
    if (input.source.type === "automatic") {
      ...
      validateIcon(bytes);
      await writeFileAtomic(cachePath(input.ramblaHome, input.projectId), bytes);
...
export class ProjectIconReader {
  ...
  constructor(private readonly ramblaHome: string) {}

  async snapshot(project: PersistedProjectRecord): Promise<ProjectIconSnapshot> {
    const snapshot = await readProjectIconSnapshot({ ramblaHome: this.ramblaHome, project });
...
export async function readProjectIconSnapshot(input: {
  ...
  if (input.project.customIconRevision) {
    ...
    try {
      icon = validateIcon(await readFile(cachePath(input.ramblaHome, input.project.projectId)));
    } catch {
...
export async function removeProjectCustomIcon(input: {
  ...
  await rm(cachePath(input.ramblaHome, input.projectId), { force: true });
}
...
function cachePath(ramblaHome: string, projectId: string): string {
  const key = createHash("sha256").update(projectId).digest("hex");
  return join(ramblaHome, "projects", "icons", `${key}.bin`);
}
