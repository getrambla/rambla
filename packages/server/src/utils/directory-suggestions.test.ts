...
describe("searchDirectoryEntries", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(searchRoot, "projects", "rambla-desktop"), { recursive: true });
  ...
  it("applies result paths and entry kinds as parameters of one search", async () => {
    ...
    expect({ directories, files }).toEqual({
      directories: [
        {
          path: path.join(searchRoot, "projects", "rambla-desktop"),
          kind: "directory",
        },
      ],
      files: [
  ...
  it("matches rooted queries against the full path at any depth", async () => {
    ...
    const expected = [
      ...
      { path: "projects/rambla-desktop", kind: "directory" },
    ];
  ...
  it("does not spend the scan budget on excluded entry kinds", async () => {
    ...
    const target = path.join(budgetRoot, "z-projects", "rambla-target");
    ...
    ).resolves.toEqual([{ path: "z-projects/rambla-target", kind: "directory" }]);
  });
...
describe("absolute directory-path configuration", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(homeDir, "projects", "rambla"), { recursive: true });
  ...
  it("shares the scan budget fairly between nested sibling branches", async () => {
    ...
    const projectPath = path.join(budgetHome, "work", "client", "team", "rambla-desktop");
  ...
  it.skipIf(isWindows)("does not let a queued symlink hide the direct project branch", async () => {
    ...
    const projectPath = path.join(projectRoot, "rambla-desktop");
  ...
  it.skipIf(isWindows)("follows visible directory symlinks that stay inside home", async () => {
    ...
    const projectPath = path.join(symlinkHome, ".linked", "project-root", "rambla-desktop");
    mkdirSync(projectPath, { recursive: true });
  ...
  it.skipIf(isWindows)("matches the visible name of a directory symlink", async () => {
    ...
    const visibleProjectPath = path.join(projectsPath, "rambla");
  ...
  it("supports home-relative path query syntax", async () => {
    ...
    expect(result.map((entry) => realpathSync.native(entry))).toEqual([
      realpathSync.native(path.join(homeDir, "projects", "rambla")),
...
describe("relative typed-entry configuration", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(path.join(workspaceDir, "README.md"), "# rambla\n");
    writeFileSync(
  ...
  it("suffix mode matches whole path segment suffixes without fuzzy matches", async () => {
    ...
    writeFileSync(path.join(workspaceDir, "src", "rambla-config-file.ts"), "");
  ...
  it("suffix mode resolves explicit hidden file paths without broad hidden traversal", async () => {
    const targetPath = path.join(workspaceDir, ".dev", "rambla-home", "daemon.log");
    mkdirSync(path.dirname(targetPath), { recursive: true });
    writeFileSync(targetPath, "daemon log\n");

    const results = await searchRelativeDirectoryEntries({
      cwd: workspaceDir,
      query: ".dev/rambla-home/daemon.log",
      limit: 20,
      includeFiles: true,
      includeDirectories: false,
      matchMode: "suffix",
    ...
    expect(results).toEqual([{ path: ".dev/rambla-home/daemon.log", kind: "file" }]);
  });
  ...
  it("matches a path fragment anywhere in the full file path", async () => {
    ...
    expect(results).toEqual([
      {
        path: "something/something-else/skills/rambla-advisor/SKILL.md",
        kind: "file",
      },
    ]);
  });
