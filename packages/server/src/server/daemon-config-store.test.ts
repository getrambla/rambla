...
describe("DaemonConfigStore", () => {
  ...
  test("patch persists relay state and emits its field change", () => {
    const ramblaHome = mkdtempSync(path.join(tmpdir(), "rambla-daemon-config-store-"));
    tempDirs.push(ramblaHome);
    const store = new DaemonConfigStore(ramblaHome, {
      relay: { enabled: false },
    ...
    expect(loadPersistedConfig(ramblaHome).daemon?.relay?.enabled).toBe(true);
  });
  ...
  test("patch round-trips agent profiles through the strictly-parsed persisted config", () => {
    ...
    expect(loadPersistedConfig(ramblaHome).daemon?.agentProfiles).toEqual([
      {
        id: "profile_ui",
        name: "UI work",
        icon: "🎨",
  ...
  test("patch replaces the whole agent profile list rather than merging entries", () => {
    ...
    expect(loadPersistedConfig(ramblaHome).daemon?.agentProfiles).toHaveLength(1);
  });
  ...
  test("rolls back config when a field transition fails", () => {
    ...
    expect(loadPersistedConfig(ramblaHome).daemon?.relay?.enabled).toBe(false);
  });
  ...
  test("rolls back live owners when a later transactional owner fails", () => {
    ...
    expect(loadPersistedConfig(ramblaHome).daemon?.browserTools?.enabled).toBeUndefined();
  });
  ...
  test("unrelated patches persist only requested file intent", () => {
    const ramblaHome = mkdtempSync(path.join(tmpdir(), "rambla-daemon-config-store-"));
    tempDirs.push(ramblaHome);
    const before = loadPersistedConfig(ramblaHome);
    const store = new DaemonConfigStore(
      ramblaHome,
      {
        relay: { enabled: true },
    ...
    expect(loadPersistedConfig(ramblaHome)).toEqual({
      ...before,
  ...
  test("patch persists provider enabled flags into config.json", () => {
    ...
    const initial = loadPersistedConfig(ramblaHome);
  ...
  test("patch persists provider Rambla-tool policy without changing availability", () => {
    ...
    store.patch({
      providers: {
        claude: {
          ramblaTools: { enabled: true, disabledTools: ["list_agents"] },
        },
      },
    });
    store.patch({
      providers: {
        claude: {
          ramblaTools: { disabledTools: ["create_agent"] },
        },
      },
    });
    ...
    expect(store.get().providers.claude).toEqual({
      enabled: false,
      ramblaTools: { enabled: true, disabledTools: ["create_agent"] },
    });
    expect(loadPersistedConfig(ramblaHome).agents?.providers?.claude).toEqual({
      enabled: false,
      ramblaTools: { enabled: true, disabledTools: ["create_agent"] },
    });
  });
  ...
  test("patch persists custom ACP provider overrides into config.json", () => {
    ...
    const persisted = loadPersistedConfig(ramblaHome);
    expect(persisted.agents?.providers?.["rambla-e2e-acp"]).toEqual({
      extends: "acp",
      label: "Rambla E2E ACP",
      description: "E2E ACP provider fixture",
      command: ["npx", "-y", "--version"],
...
describe("DaemonConfigStore reload", () => {
  ...
  function createReloadableStore(
    ...
    const ramblaHome = mkdtempSync(path.join(tmpdir(), "rambla-daemon-config-reload-"));
    tempDirs.push(ramblaHome);
    ...
    const store = new DaemonConfigStore(ramblaHome, initialMutable, undefined, {
      reloadSource: {
    ...
    return { ramblaHome, store, persisted };
  }

  function writeConfig(ramblaHome: string, config: unknown): void {
    writeFileSync(path.join(ramblaHome, "config.json"), `${JSON.stringify(config, null, 2)}\n`);
  }

  test("applies mutable edits and reports startup-only edits", () => {
    const { ramblaHome, store, persisted } = createReloadableStore();
    writeConfig(ramblaHome, {
      ...persisted,
      daemon: {
        ...persisted.daemon,
  ...
  test("applies the global plugin switch in both directions", () => {
    ...
    writeConfig(ramblaHome, { ...persisted, pluginsEnabled: true });
    expect(store.reload()).toEqual({
    ...
    writeConfig(ramblaHome, { ...persisted, pluginsEnabled: false });
    expect(store.reload()).toEqual({
      appliedPaths: ["pluginsEnabled"],
      restartRequiredPaths: [],
      overrideControlledPaths: [],
  ...
  test("classifies every leaf when a parent subtree is added", () => {
    const { ramblaHome, store } = createReloadableStore({
  ...
  test("classifies every leaf when the daemon subtree is removed", () => {
    ...
    writeConfig(ramblaHome, { version: 1 });
  ...
  test("invalid JSON and invalid schema apply nothing", () => {
    const { ramblaHome, store } = createReloadableStore();
    writeFileSync(path.join(ramblaHome, "config.json"), "{ nope\n");
    expect(() => store.reload()).toThrow("Invalid JSON");
    expect(store.get().browserTools.enabled).toBe(false);

    writeConfig(ramblaHome, { daemon: { browserTools: { enabled: "yes" } } });
  ...
  test("removing providers and optional profiles clears live state", () => {
    ...
    writeConfig(ramblaHome, persisted);
