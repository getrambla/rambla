...
function createArchiveDeps(input: ArchiveDepsInput): ArchiveTestDependencies {
  ...
  return {
    ...
    ramblaWorktreesBaseRoot: input.ramblaWorktreesBaseRoot,
...
describe("archiveByScope", () => {
  test("workspace scope archives the record and removes the directory on last reference", async () => {
    ...
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "last-ref-workspace");
  ...
  test("workspace scope runs teardown while keeping a directory referenced by a sibling", async () => {
    ...
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "sibling-workspace");
  ...
  test("workspace scope skips teardown while repository automation is blocked", async () => {
    ...
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "blocked-teardown");
  ...
  test("workspace scope keeps a worktree for an active workspace in a subdirectory", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "subdirectory-sibling");
  ...
  test("archiving a subdirectory workspace keeps its active worktree root", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "subdirectory-target");
  ...
  test("workspace scope runs teardown from the exact nested workspace before deleting its worktree", async () => {
    ...
    writeFileSync(
      path.join(sourceNested, "rambla.json"),
      JSON.stringify({
        worktree: {
          teardown: [
    ...
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "nested-teardown");
  ...
  test("worktree scope archives root and subdirectory workspaces before removing the backing worktree", async () => {
    ...
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "worktree-scope");
  ...
  test("workspace scope never removes a non-Rambla-owned directory", async () => {
  ...
  test("worktree scope keeps the directory when one record teardown fails", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "partial-failure");
  ...
  test("worktree scope removes an owned directory with zero matching records", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "zero-records");
  ...
  test("marks archiving, emits an upsert carrying the archiving state, then clears it and emits a remove", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "lifecycle");
  ...
  test("archives stored snapshots only for the target workspace", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "snapshot-scope");
  ...
  test("worktree scope archives three workspaces on the directory and removes it", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const ramblaHome = path.join(tempDir, ".rambla");
    const worktree = await createRamblaOwnedWorktree(repoDir, ramblaHome, "worktree-scope-n3");
