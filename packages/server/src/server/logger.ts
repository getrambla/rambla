...
function resolveFilePath(ramblaHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(ramblaHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(ramblaHome, configuredPath);
}
...
function resolveConfiguredRamblaHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.ramblaHome) {
    return options.ramblaHome;
  }
...
export function resolveLogConfig(
  ...
  const ramblaHome = resolveConfiguredRamblaHome(options);
  ...
  const file =
    options?.file !== false && persistedLog?.file
      ? {
          ...
          path: resolveFilePath(ramblaHome, persistedLog.file.path),
        }
      : undefined;
