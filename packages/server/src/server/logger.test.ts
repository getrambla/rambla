...
async function runLoggerFixture(source: string): Promise<{ stdout: string; stderr: string }> {
  const tempDir = await mkdtemp(path.join(tmpdir(), "rambla-logger-fixture-"));
...
describe("resolveLogConfig", () => {
  const ramblaHome = "/tmp/rambla-logger-tests";

  it("defaults to stdout JSON without file logging", () => {
    const result = resolveLogConfig(undefined, { ramblaHome });
  ...
  it("keeps legacy level and format as stdout configuration", () => {
    const result = resolveLogConfig({ level: "warn", format: "pretty" }, { ramblaHome });
  ...
  it("enables file output only when log.file is present", () => {
    ...
    expect(resolveLogConfig(config, { ramblaHome })).toEqual({
      level: "debug",
      console: {
        level: "warn",
        format: "json",
      ...
      file: {
        level: "debug",
        path: path.resolve(ramblaHome, "logs", "programmatic.log"),
      },
    });
  });
  ...
  it("defaults file output to info when log.file is present without a level", () => {
    ...
    expect(resolveLogConfig(config, { ramblaHome })).toEqual({
      ...
      file: {
        level: "info",
        path: path.resolve(ramblaHome, "daemon.log"),
      },
    });
  });
});
...
describe("loadConfig logger config", () => {
  it("applies log format env at the config boundary", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "rambla-logger-config-"));
    ...
    const config = loadConfig(ramblaHome, {
      env: { RAMBLA_LOG_FORMAT: "pretty" },
    });

    expect(config.log?.format).toBe("pretty");
    expect(resolveLogConfig(config, { ramblaHome }).console.format).toBe("pretty");
  });
});

describe("createRootLogger", () => {
  it("includes the daemon version in child logger entries", async () => {
    const ramblaHome = await mkdtemp(path.join(tmpdir(), "rambla-logger-version-"));

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(undefined, { ramblaHome: ${JSON.stringify(ramblaHome)} });
  ...
  it("writes JSON to stdout by default and does not initialize file logging", async () => {
    const ramblaHome = await mkdtemp(path.join(tmpdir(), "rambla-logger-default-"));
    const missingLogDir = path.join(ramblaHome, "logs");
    ...
    expect(existsSync(path.join(ramblaHome, "daemon.log"))).toBe(false);
    expect(existsSync(missingLogDir)).toBe(false);
  });

  it("writes to an explicit file target without creating rotation files", async () => {
    const ramblaHome = await mkdtemp(path.join(tmpdir(), "rambla-logger-file-"));
    const logPath = path.join(ramblaHome, "logs", "programmatic.log");

    await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { ramblaHome: ${JSON.stringify(ramblaHome)} },
      );
  ...
  it("can disable file output for supervised workers", async () => {
    const ramblaHome = await mkdtemp(path.join(tmpdir(), "rambla-logger-no-worker-file-"));
    const logPath = path.join(ramblaHome, "daemon.log");

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { ramblaHome: ${JSON.stringify(ramblaHome)}, file: false },
      );
  ...
  it("keeps pretty output available as a format choice", async () => {
    const ramblaHome = await mkdtemp(path.join(tmpdir(), "rambla-logger-pretty-"));
