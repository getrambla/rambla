...
describe("daemon web UI config", () => {
  ...
  test("RAMBLA_WEB_UI_ENABLED overrides persisted setting", async () => {
    const home = await createRamblaHome({
      version: 1,
      features: { webUi: { enabled: true } },
    });

    const config = loadConfig(home, { env: { RAMBLA_WEB_UI_ENABLED: "false" } });

    expect(config.webUi.enabled).toBe(false);
  });

  test("RAMBLA_WEB_UI_ENABLED=true enables web UI", async () => {
    const home = await createRamblaHome({ version: 1 });

    const config = loadConfig(home, { env: { RAMBLA_WEB_UI_ENABLED: "true" } });
  ...
  test("CLI web UI enable override wins over env and persisted config", async () => {
    ...
    const config = loadConfig(home, {
      env: { RAMBLA_WEB_UI_ENABLED: "false" },
  ...
  test("CLI web UI disable override wins over env and persisted config", async () => {
    ...
    const config = loadConfig(home, {
      env: { RAMBLA_WEB_UI_ENABLED: "true" },
  ...
  test("resolves relative persisted distDir against RAMBLA_HOME", async () => {
    const home = await createRamblaHome({
      version: 1,
  ...
  test("RAMBLA_WEB_UI_DIST_DIR overrides persisted distDir", async () => {
    const home = await createRamblaHome({
      version: 1,
    ...
    const config = loadConfig(home, { env: { RAMBLA_WEB_UI_DIST_DIR: envDir } });
