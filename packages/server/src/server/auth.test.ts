...
describe("daemon bearer validator", () => {
  ...
  test("extracts WebSocket rambla bearer subprotocol tokens", () => {
    const protocol = extractWsBearerProtocol("chat, rambla.bearer.secret.with.dots");

    expect(protocol).toBe("rambla.bearer.secret.with.dots");
    expect(extractWsBearerToken(protocol)).toBe("secret.with.dots");
    expect(extractWsBearerToken("rambla.other.secret")).toBeNull();
  });
