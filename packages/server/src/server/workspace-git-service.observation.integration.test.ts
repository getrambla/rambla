...
test("recursive observation updates tracked state and prunes ignored storms", async () => {
  const tempDir = realpathSync(mkdtempSync(path.join(tmpdir(), "rambla-git-observation-")));
...
test("a late Git-ignored tree is pruned while tracked changes still notify consumers", async () => {
  const tempDir = realpathSync(mkdtempSync(path.join(tmpdir(), "rambla-real-ignore-")));
  const repoDir = path.join(tempDir, "repo");
  mkdirSync(repoDir);
  ...
  const service = new WorkspaceGitServiceImpl({
    ...
    ramblaHome: path.join(tempDir, "home"),
    fileObserver,
  });
