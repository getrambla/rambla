...
async function readNativeThreadPath(threadId: string): Promise<string> {
  ...
  try {
    await client.request("initialize", {
      clientInfo: { name: "rambla-archive-regression", version: "1.0.0" },
    });
...
// archived by older Rambla versions whose best-effort native archive failed.
test.runIf(process.env.RAMBLA_NATIVE_ARCHIVE_QA === "1").each([true, false])(
...
test.runIf(process.env.RAMBLA_NATIVE_ARCHIVE_QA === "1")(
