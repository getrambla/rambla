...
describe("ACPAgentSession terminal tools", () => {
  ...
  describe("agent launch environment", () => {
    ...
    const readLaunchIdentityArgs = [
      "-e",
      "process.stdout.write(JSON.stringify({ id: process.env.RAMBLA_AGENT_ID ?? null, cwd: process.env.RAMBLA_AGENT_CWD ?? null, extra: process.env.RAMBLA_TEST_EXTRA ?? null }))",
    ];
    ...
    test("gives the terminal the agent's identity", async () => {
      // The daemon itself does not run inside a Rambla agent.
      vi.stubEnv("RAMBLA_AGENT_ID", undefined);
      vi.stubEnv("RAMBLA_AGENT_CWD", undefined);
      const session = createSession({
        launchEnv: {
          RAMBLA_AGENT_ID: "agent-1",
          RAMBLA_AGENT_CWD: "/repo",
        },
      });
    ...
    test("prefers the agent's identity over the daemon's own environment", async () => {
      // A daemon started from inside another Rambla agent carries that agent's id.
      vi.stubEnv("RAMBLA_AGENT_ID", "daemon-host-agent");
      vi.stubEnv("RAMBLA_AGENT_CWD", "/elsewhere");
      const session = createSession({
        launchEnv: {
          RAMBLA_AGENT_ID: "agent-1",
          RAMBLA_AGENT_CWD: "/repo",
    ...
    test("lets the requested terminal environment win over the launch environment", async () => {
      const session = createSession({
        launchEnv: {
          RAMBLA_AGENT_ID: "agent-1",
          RAMBLA_TEST_EXTRA: "from-launch",
        },
      });

      await expect(
        readLaunchIdentity(session, {
          env: [{ name: "RAMBLA_TEST_EXTRA", value: "from-request" }],
        }),
    ...
    test("carries the agent's identity into single-string shell commands", async () => {
      const child = createTerminalChildStub();
      const spawn = vi.spyOn(spawnUtils, "spawnProcess").mockReturnValue(child);
      const session = createSession({ launchEnv: { RAMBLA_AGENT_ID: "agent-1" } });

      await session.createTerminal({
        sessionId: "session-1",
        command: "rambla heartbeat create --every 5m",
        cwd: "/repo",
      });
