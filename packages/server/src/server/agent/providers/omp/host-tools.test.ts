...
function createCatalog(tools: RamblaToolDefinition[]): RamblaToolCatalog {
...
class OmpHostToolHarness {
  ...
  private resolveControlledResult: ((result: RamblaToolResult) => void) | null = null;
  ...
  private constructor(
    private readonly catalog: RamblaToolCatalog,
    private readonly runtimeSession: Awaited<ReturnType<FakeOmp["startSession"]>>,
  ) {}

  static async withTools(tools: RamblaToolDefinition[]): Promise<OmpHostToolHarness> {
  ...
  static async cancellable(): Promise<OmpHostToolHarness> {
    ...
    const tool: RamblaToolDefinition = {
      name: "wait_for_agent",
      description: "Wait for a Rambla agent.",
      handler: async (_input, context) => {
        harness.controlledSignal = context.signal ?? null;
        harness.resolveControlledStart?.();
        return await new Promise<RamblaToolResult>((resolve) => {
  ...
  completeControlledCall(result: RamblaToolResult): void {
...
describe("OMP host tools", () => {
  test("marks every caller-scoped Rambla tool essential for direct invocation", () => {
