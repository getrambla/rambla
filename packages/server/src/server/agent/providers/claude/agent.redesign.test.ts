...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "RAMBLA_ENV_SENTINEL_SECRET";
  const runtimeSecret = "RAMBLA_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "RAMBLA_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.RAMBLA_TEST_SENTINEL_SECRET;
  process.env.RAMBLA_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        RAMBLA_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.RAMBLA_TEST_SENTINEL_SECRET;
    } else {
      process.env.RAMBLA_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
