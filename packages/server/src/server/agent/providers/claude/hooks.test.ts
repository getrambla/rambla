...
describe("mergeClaudeHooks", () => {
  ...
  it("keeps Rambla's hooks when the user configured none", () => {
  ...
  it("appends user hooks for the same event instead of replacing them", () => {
    ...
    // Both must survive: dropping Rambla's would silently stop effort being observed, and
    // dropping the user's would break their configuration.
    expect(merged.PreToolUse).toHaveLength(2);
  });

  it("carries through events Rambla does not register", () => {
