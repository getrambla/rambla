...
async function createRealHarness() {
  const artifactDir = mkdtempSync(path.join(os.tmpdir(), "rambla-opencode-recovery-"));
  const runtime = createDisposableRuntime();
  const { runtimeDir, home, ramblaHome, xdgConfig, xdgData, xdgCache, xdgState, workspace } =
    runtime;
...
function createDisposableRuntime(
  ...
  try {
    runtimeDir = mkdtempSync(path.join(os.tmpdir(), "rambla-opencode-runtime-"));
    onCreate?.(runtimeDir);
    ...
    for (const directory of [home, ramblaHome, xdgConfig, xdgData, xdgCache, xdgState, workspace]) {
    ...
    return { runtimeDir, home, ramblaHome, xdgConfig, xdgData, xdgCache, xdgState, workspace };
  } catch (error) {
    if (runtimeDir) {
      rmSync(runtimeDir, { recursive: true, force: true });
    }
