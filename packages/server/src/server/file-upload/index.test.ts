...
describe("file uploads", () => {
  ...
  it("stores chunked upload bytes and returns an uploaded-file attachment", async () => {
    const ramblaHome = makeRamblaHome();
    const uploads = new FileUploadStore({ ramblaHome });
    ...
    const path = uploadedPath(ramblaHome, "notes.txt");
  ...
  it("keeps the original file name for non-ASCII and punctuated names", async () => {
    const uploads = new FileUploadStore({ ramblaHome: makeRamblaHome() });
  ...
  it("preserves chunk order when frames arrive before earlier disk writes finish", async () => {
    ...
    expect(readFileSync(uploadedPath(ramblaHome, "notes.txt"), "utf8")).toBe("hello world");
  });
  ...
  it("replaces duplicate upload starts without letting the old stale timeout evict the replacement", async () => {
    ...
    const ramblaHome = makeRamblaHome();
    const uploads = new FileUploadStore({ ramblaHome, staleUploadTimeoutMs: 50 });
    ...
    const path = uploadedPath(ramblaHome, "new.txt");
...
function uploadedPath(ramblaHome: string, fileName: string): string {
  const root = join(ramblaHome, "uploads");
