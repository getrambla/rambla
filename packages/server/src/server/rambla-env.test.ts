...
describe("rambla env contract", () => {
  ...
  const baseEnv = {
    ...
    PATH: "/usr/bin",
    RAMBLA_AGENT_ID: "agent-123",
    RAMBLA_DESKTOP_MANAGED: "1",
    [RAMBLA_NODE_ENV]: "production",
    RAMBLA_SUPERVISED: "1",
  ...
  test("builds external process env by scrubbing runtime control vars after overlays", () => {
    ...
    expect(env.RAMBLA_AGENT_ID).toBe("agent-123");
  ...
  test("builds external command env without process.execPath special-casing", () => {
    ...
    expect(env.RAMBLA_DESKTOP_MANAGED).toBeUndefined();
    expect(env[RAMBLA_NODE_ENV]).toBeUndefined();
    expect(env.RAMBLA_SUPERVISED).toBeUndefined();
  });
  ...
  test("builds self node command with Electron node mode", () => {
    ...
    expect(command.env.RAMBLA_DESKTOP_MANAGED).toBeUndefined();
    expect(command.env[RAMBLA_NODE_ENV]).toBeUndefined();
    expect(command.env.RAMBLA_SUPERVISED).toBeUndefined();
  });
