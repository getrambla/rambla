...
function seedFetchFixture(): {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), "rambla-noop-fetch-"));
  cleanupPaths.push(fixtureRoot);
  const repoRoot = join(fixtureRoot, "repo");
  const originRoot = join(fixtureRoot, "origin.git");
  const ramblaHome = join(fixtureRoot, "rambla-home");
  ...
  return { originRoot, ramblaHome, repoRoot, worktrees };
}
