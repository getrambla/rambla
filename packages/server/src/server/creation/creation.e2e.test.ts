...
import { SessionInboundMessageSchema, WSOutboundMessageSchema } from "@getrambla/protocol/messages";
...
import type { CreationSnapshot, SessionOutboundMessage } from "@getrambla/protocol/messages";
...
test("legacy keyed creation preserves checkout error codes", async () => {
  ...
  const daemon = await createTestRamblaDaemon({ agentClients: createTestAgentClients() });
  const peer = await connectCreationPeer(daemon.port);
  try {
