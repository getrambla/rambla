...
describe("PluginService", () => {
  ...
  it("lists manifest descriptions for running and disabled plugins without hiding malformed entries", async () => {
    ...
    await writeFile(
      path.join(running, "rambla-plugin.json"),
      JSON.stringify({ id: "running", description: "Runs checks" }),
    );
    await writeFile(
      path.join(disabled, "rambla-plugin.json"),
      JSON.stringify({ id: "disabled", description: "Waits until enabled" }),
    );
    await writeFile(path.join(malformed, "rambla-plugin.json"), "{");
    const service = createService(home, {
  ...
  it.each([true, false])(
    ...
    async (enabled) => {
      const home = await mkdtemp(path.join(tmpdir(), "rambla-legacy-layout-"));
      roots.push(home);
      const repository = await mkdtemp(path.join(tmpdir(), "rambla-legacy-remote-"));
      roots.push(repository);
      const id = "legacy-review";
      const pluginPath = "plugins/review";
      await mkdir(path.join(repository, pluginPath), { recursive: true });
      await writeFile(
        path.join(repository, pluginPath, "rambla-plugin.json"),
        JSON.stringify({ id, requirements: { rambla: ">=0.8.0" } }),
      );
      await writeFile(
...
export default function contribute(server: PluginServerContext) {
  ...
  it.each([false, true])(
    ...
    async (failRestore) => {
      const home = await mkdtemp(path.join(tmpdir(), "rambla-plugin-recovery-"));
      roots.push(home);
...
export default function contribute(plugin: unknown) {
  ...
  it("reports invalid manifests, missing entries, and startup failures", async () => {
    ...
    await expect(service.installDirectory({ path: legacy })).rejects.toThrow(
      "This plugin was made for an older version of Rambla and cannot run on Rambla v0.8. Ask its author to update it. Plugin authors can follow the migration guide: https://rambla.sh/docs/plugins/migration",
    );
...
describe("npm plugin installation", () => {
  it("preserves dependencies and metadata, and reloads without npm", async () => {
    ...
    const failures: Array<[string, Record<string, string>]> = [
      ["bad-manifest", { "rambla-plugin.json": "{}" }],
      [
        "bad-build",
        {
          "rambla-plugin.json": JSON.stringify({
            id: "bad-build",
            requirements: { rambla: ">=0.4.0" },
    ...
    const registry = await startNpmRegistry([
      ...packages,
      {
        name: "rambla-prepared-plugin",
        version: "1.0.0",
        dependencies: { "rambla-fixture-dependency": "1.0.0" },
        scripts: { postinstall: "node -e \"throw new Error('npm lifecycle must not run')\"" },
        files: {
          "nested/rambla-plugin.json": JSON.stringify({
            id: "prepared",
            requirements: { rambla: ">=0.4.0" },
            build: [
              [
...
import dependency from "rambla-fixture-dependency";
import { z } from "zod";
export default function contribute() {
  ...
  return () => {};
    ...
    const home = await mkdtemp(path.join(tmpdir(), "rambla-npm-service-"));
    roots.push(home);
    ...
    try {
      await service.start();
      const installed = await service.installSource({ source: "npm:rambla-fixture-plugin@^1.0.0" });
      expect(installed).toMatchObject({
        id: "npm-review",
        status: "running",
        installation: {
          identity: { kind: "npm", packageName: "rambla-fixture-plugin", pluginPath: "." },
      ...
      expect(
        JSON.parse(await readFile(path.join(installRoot, "package-lock.json"), "utf8")).packages[
          "node_modules/rambla-fixture-dependency"
        ].version,
      ).toBe("1.0.0");
      for (const [source, id, version] of [
        ["npm:rambla-fixture-plugin@1.0.0", "exact", "1.0.0"],
        ["rambla-fixture-plugin@stable", "tag", "1.0.0"],
        ["npm:@rambla-fixture/review", "scoped", "2.0.0"],
      ]) {
      ...
      registry.setTag("rambla-fixture-plugin", "latest", "2.0.0");
      ...
      registry.setTag("rambla-fixture-plugin", "latest", "1.1.0");
      ...
      const prepared = await service.installSource({
        source: "npm:rambla-prepared-plugin@1.0.0:nested",
      });
      ...
      expect(registry.requests).not.toContain("/@getrambla/plugin");
      await expect(service.installSource({ source: "npm:missing-plugin" })).rejects.toThrow();
      await expect(
        service.installSource({ source: "npm:rambla-fixture-plugin", ref: "main" }),
      ).rejects.toThrow("--ref is only valid for Git");
      await expect(service.installSource({ source: "npm:rambla-fixture-plugin" })).rejects.toThrow(
        "already configured",
      );
      expect(await readdir(path.join(home, "plugins", ".staging"))).toEqual([]);
      for (const name of [
      ...
      await expect(
        service.installSource({ source: "npm:rambla-fixture-plugin", id: "missing-npm" }),
