...
import { z } from "zod";
import { defineRpc, settingsRpc } from "@getrambla/plugin";
...
export default function(server) {
  ...
  const options = {
    ramblaHomeRoot: path.join(root, "daemon"),
  ...
  const first = await createTestRamblaDaemon(options);
  ...
  const restarted = await createTestRamblaDaemon(options);
...
test("two clients share settings, observe changes, and preserve values through plugin lifecycle", async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(directory, "index.server.ts"),
      `import { defineRpc, defineSettings } from "@getrambla/plugin";
import { z } from "zod";
