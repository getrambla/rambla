...
test("plugin host APIs and observations recover after repeated daemon-side socket closes", async () => {
  const pluginDirectory = await mkdtemp(path.join(tmpdir(), "rambla-reconnecting-plugin-"));
  const workspaceDirectory = await mkdtemp(path.join(tmpdir(), "rambla-reconnecting-workspace-"));
  roots.push(pluginDirectory, workspaceDirectory);
  await writeFile(
    path.join(pluginDirectory, "rambla-plugin.json"),
    JSON.stringify({ id: "reconnecting", requirements: { rambla: ">=0.8.0" } }),
  );
  await writeFile(
    path.join(pluginDirectory, "index.server.ts"),
    `
...
export default function contribute(server: PluginServerContext) {
  ...
  server.handle(defineRpc({ name: "observe", input: z.object({}), output: z.string() }), async (_, { rambla }) => {
    const { subscription } = await rambla.workspaces.list({ subscribe: {} });
  ...
  server.handle(defineRpc({ name: "probe", input: z.object({}), output: z.object({ pid: z.number(), projectIds: z.array(z.string()) }) }), async (_, { rambla }) => {
    await reconnected;
    return { pid: process.pid, projectIds: (await rambla.projects.list()).projects.map((project) => project.projectId) };
  });
  ...
  server.handle(defineRpc({ name: "disconnect", input: z.object({}), output: z.null() }), () => new Promise((resolve) => {
    ...
    function closed(message: { type: string }) {
      if (message.type !== "rambla_close") return;
    ...
    process.send!({ type: "rambla_frame", isBinary: false, data: JSON.stringify({
