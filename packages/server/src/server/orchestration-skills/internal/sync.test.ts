...
async function makeSandbox(): Promise<Sandbox> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "rambla-skill-sync-"));
...
describe("syncSkills", () => {
  ...
  it("overwrites on-disk skill content when the bundle differs", async () => {
    await writeBundleSkill(sandbox.sourceDir, "rambla", {
      "SKILL.md": "new rambla content",
    });
    const onDiskSkill = path.join(sandbox.agentsDir, "rambla");
    await fs.mkdir(onDiskSkill, { recursive: true });
    await fs.writeFile(path.join(onDiskSkill, "SKILL.md"), "old rambla content");
    ...
    const agentsContent = await fs.readFile(
      path.join(sandbox.agentsDir, "rambla", "SKILL.md"),
      "utf-8",
    );
    expect(agentsContent).toBe("new rambla content");
    const claudeContent = await fs.readFile(
      path.join(sandbox.claudeDir, "rambla", "SKILL.md"),
      "utf-8",
    );
  ...
  it("installs new bundled skills, including references/, when not present on disk", async () => {
    await writeBundleSkill(sandbox.sourceDir, "rambla-committee", {
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["rambla-committee"],
    });

    expect(
      await fs.readFile(path.join(sandbox.agentsDir, "rambla-committee", "SKILL.md"), "utf-8"),
    ).toBe("committee content");
    expect(
      await fs.readFile(
        path.join(sandbox.agentsDir, "rambla-committee", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");
    expect(
  ...
  it("leaves on-disk skills not in the bundle untouched", async () => {
    await writeBundleSkill(sandbox.sourceDir, "rambla", { "SKILL.md": "new" });
  ...
  it("removes stale files previously written to managed skill dirs", async () => {
    ...
    await fs.rm(path.join(sandbox.sourceDir, "rambla"), { recursive: true, force: true });
    await writeBundleSkill(sandbox.sourceDir, "rambla", { "SKILL.md": "new" });
  ...
  it("skips skills listed in skillNames that are missing from the bundle without raising", async () => {
    await writeBundleSkill(sandbox.sourceDir, "rambla", { "SKILL.md": "content" });
    // "rambla-removed" is in skillNames but not in the bundle on disk.
    ...
    const result = await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["rambla", "rambla-removed"],
      onSkillError: (name, error) => errors.push({ name, error }),
    });

    expect(errors).toEqual([]);
    expect(result.processedSkills).toBe(1);
    expect(await fs.readFile(path.join(sandbox.agentsDir, "rambla", "SKILL.md"), "utf-8")).toBe(
      "content",
    );
    await expect(fs.access(path.join(sandbox.agentsDir, "rambla-removed"))).rejects.toThrow();
  });

  it("leaves on-disk skill content alone when the skill has been removed from the bundle", async () => {
    await writeBundleSkill(sandbox.sourceDir, "rambla", { "SKILL.md": "current" });
    const deprecatedDir = path.join(sandbox.agentsDir, "rambla-deprecated");
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["rambla", "rambla-deprecated"],
    });
  ...
  it("reports per-skill errors via onSkillError without throwing", async () => {
    ...
    await fs.writeFile(path.join(sandbox.agentsDir, "rambla"), "blocking file");
    ...
    expect(errors).toContain("rambla");
...
describe("removeSkill", () => {
  ...
  it("removes the skill from all three targets when present", async () => {
    ...
    await removeSkill("rambla", {
      agentsDir: sandbox.agentsDir,
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
    });

    await expect(fs.access(path.join(sandbox.agentsDir, "rambla"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.claudeDir, "rambla"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.codexDir, "rambla"))).rejects.toThrow();
  });
