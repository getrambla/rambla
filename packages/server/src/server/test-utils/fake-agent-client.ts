...
class FakeAgentSession implements AgentSession {
  ...
  private async runSlashCommand(
    ...
    if (this.providerName === "codex" && fullName.startsWith("prompts:")) {
      ...
      return {
        text: `RAMBLA_OK ${args ?? ""}`.trim(),
        timeline: [{ type: "assistant_message", text: `RAMBLA_OK ${promptId}` }],
        usage: { inputTokens: 1, outputTokens: 1 },
      };
    }

    return {
      text: "RAMBLA_SKILL_OK",
      timeline: [{ type: "assistant_message", text: "RAMBLA_SKILL_OK" }],
      usage: { inputTokens: 1, outputTokens: 1 },
    };
  }
