...
function seedSameCwdWorkspaces(): { ramblaHomeRoot: string; cwd: string } {
  const ramblaHomeRoot = mkdtempSync(path.join(tmpdir(), "rambla-same-cwd-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "rambla-same-cwd-dir-"));
  const projectsDir = path.join(ramblaHomeRoot, ".rambla", "projects");
  ...
  return { ramblaHomeRoot, cwd };
}

async function seedWorkspaceWithLegacyAgent(): Promise<{ ramblaHomeRoot: string; cwd: string }> {
  const ramblaHomeRoot = mkdtempSync(path.join(tmpdir(), "rambla-legacy-agent-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "rambla-legacy-agent-dir-"));
  ...
  const agentStorage = new AgentStorage(path.join(ramblaHome, "agents"), createTestLogger());
...
test("daemon bootstrap migrates cwd-only legacy agents before same-cwd workspaces are added", async () => {
  const { ramblaHomeRoot, cwd } = await seedWorkspaceWithLegacyAgent();
  const daemon = await createTestRamblaDaemon({ ramblaHomeRoot });
...
test("workspace.create directory source with firstAgentContext generates a daemon-visible workspace title", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "rambla-named-local-dir-"));
...
test("local workspace auto-title does not broadcast provider snapshot warm-up to clients", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "rambla-title-snapshot-storm-"));
...
test("create_agent_request with workspaceId does not retitle an existing workspace", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "rambla-agent-submit-title-"));
  const daemon = await createTestRamblaDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("creating another same-cwd local workspace keeps running status on the owning workspace only", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "rambla-running-same-cwd-create-"));
  const daemon = await createTestRamblaDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("two workspaces sharing one cwd compute agent status per workspaceId", async () => {
  const { ramblaHomeRoot, cwd } = seedSameCwdWorkspaces();
  const daemon = await createTestRamblaDaemon({ ramblaHomeRoot });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
    appVersion: "0.1.82",
