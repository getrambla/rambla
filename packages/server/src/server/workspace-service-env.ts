...
export function buildWorkspaceServiceEnv(
  ...
  const env: Record<string, string> = {
    ...
    RAMBLA_PORT: String(selfPeer.port),
  };
  ...
  if (selfProxyUrl) {
    env.RAMBLA_URL = selfProxyUrl;
  }

  for (const peer of options.peers) {
    const envName = normalizeServiceEnvName(peer.scriptName);
    env[`RAMBLA_SERVICE_${envName}_PORT`] = String(peer.port);
    ...
    if (peerProxyUrl) {
      env[`RAMBLA_SERVICE_${envName}_URL`] = peerProxyUrl;
    }
  }
