...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "rambla-offer-home-"));
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "rambla-offer-e2e-"));
    ...
    const env = {
      ...process.env,
      RAMBLA_HOME: tempHome,
      RAMBLA_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
