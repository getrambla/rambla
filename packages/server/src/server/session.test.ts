...
const ramblaWorktreeServiceMocks = vi.hoisted(() => ({
...
vi.mock("./rambla-worktree-service.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./rambla-worktree-service.js")>();
  return {
    ...actual,
    createRamblaWorktree: ramblaWorktreeServiceMocks.createRamblaWorktree,
  };
});
...
describe("project command-center RPCs", () => {
  ...
  test("creates a directory and returns its normalized Project descriptor", async () => {
    const parentDirectory = realpathSync(mkdtempSync(join(tmpdir(), "rambla-project-session-")));
...
describe("workspace file access (behavior preservation)", () => {
  ...
  test("file upload round-trips bytes through binary frames", async () => {
    const ramblaHome = makeDir("file-access-upload-");
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({ messages, ramblaHome });
...
describe("project config RPC authorization", () => {
  ...
  test("read_project_config_request accepts the same root with a trailing slash", async () => {
    ...
    writeFileSync(join(repoRoot, "rambla.json"), JSON.stringify({ worktree: { setup: "npm ci" } }));
    const messages: unknown[] = [];
    const session = createSessionForTest({
      messages,
  ...
  test("write_project_config_request emits stale and write-failed inline domain failures", async () => {
    ...
    writeFileSync(join(staleRoot, "rambla.json"), JSON.stringify({ worktree: { setup: "old" } }));
...
describe("session checkout merge handling", () => {
  ...
  test("forces a workspace git snapshot refresh after merge-from-base succeeds", async () => {
    ...
    expect(checkoutGitMocks.mergeFromBase).toHaveBeenCalledWith(
      ...
      { ramblaHome: "/tmp/rambla-home", worktreesRoot: undefined },
    );
    expect(workspaceGitService.getSnapshot).toHaveBeenCalledWith("/tmp/request-worktree", {
      force: true,
...
`;
  ...
  function writeConfig(repoRoot: string, config: unknown): void {
    writeFileSync(join(repoRoot, "rambla.json"), `${JSON.stringify(config)}\n`);
  }

  async function generateCommitPromptWithConfig(config: unknown): Promise<string> {
    const repoRoot = makeRoot();
    if (typeof config === "string") {
      writeFileSync(join(repoRoot, "rambla.json"), config);
  ...
  test.each([
    ["rambla.json missing", undefined],
    ["rambla.json exists but invalid JSON", "{ nope"],
    ["rambla.json valid but missing metadataGeneration", {}],
...
`;
  ...
  test("keeps the PR fallback when structured generation fails", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/getrambla/rambla/pull/9",
      number: 9,
    });
    const session = createSessionForTest({ workspaceGitService, messages });
  ...
  test("forces workspace git and GitHub refresh after creating a pull request", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/getrambla/rambla/pull/2",
      number: 2,
    });
    const session = createSessionForTest({ github, workspaceGitService, messages });
...
describe("session branch validation", () => {
  ...
  test("does not validate tags as branches", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "rambla-session-branch-validation-"));
...
describe("session stash list handling", () => {
  test("lists stashes through the workspace git service", async () => {
    ...
    const entries = [
      {
        index: 0,
        message: "rambla-auto-stash: feature",
        branch: "feature",
        isRambla: true,
      },
    ];
...
describe("session stash mutation handling", () => {
  test("forces a workspace git snapshot refresh after pushing a stash", async () => {
    ...
    expect(gitCommandMocks.runGitCommand).toHaveBeenCalledWith(
      ["stash", "push", "--include-untracked", "-m", "rambla-auto-stash: feature"],
...
describe("session rambla worktree creation handling", () => {
  test("forces workspace git refreshes for the source repo and created worktree", async () => {
    const workspaceGitService = { getSnapshot: vi.fn().mockResolvedValue({}) };
    const session = createSessionForTest({ workspaceGitService });
    ramblaWorktreeServiceMocks.createRamblaWorktree.mockResolvedValue({
      repoRoot: "/tmp/repo",
      worktree: {
        branchName: "feature/new-worktree",
        worktreePath: "/tmp/rambla/worktrees/new-worktree",
      },
      workspace: {
        workspaceId: "workspace-new-worktree",
        projectId: "project-repo",
        cwd: "/tmp/rambla/worktrees/new-worktree",
        kind: "worktree",
    ...
    expect(workspaceGitService.getSnapshot).toHaveBeenCalledWith(
      "/tmp/rambla/worktrees/new-worktree",
      {
        force: true,
        reason: "create-worktree",
      },
...
describe("session workspace script handling", () => {
  test("passes the project slug and cached branch into workspace script spawning", async () => {
    ...
    const workspaceGitService = {
      ...
      getProjectSlug: vi.fn().mockResolvedValue("rambla"),
    };
...
describe("session pull request timeline handling", () => {
        ...
        items: [
          {
            ...
            url: "https://github.com/getrambla/rambla/pull/42#pullrequestreview-1",
  ...
  test.each([
    { prNumber: 0, repoOwner: "getrambla", repoName: "rambla" },
    { prNumber: -1, repoOwner: "getrambla", repoName: "rambla" },
    { prNumber: 42, repoOwner: "get rambla", repoName: "rambla" },
    { prNumber: 42, repoOwner: "getrambla/cli", repoName: "rambla" },
    { prNumber: 42, repoOwner: "get$rambla", repoName: "rambla" },
    { prNumber: 42, repoOwner: "getrambla", repoName: "pa seo" },
    { prNumber: 42, repoOwner: "getrambla", repoName: "rambla/app" },
    { prNumber: 42, repoOwner: "getrambla", repoName: "rambla!" },
  ...
  test("emits GitHub check details responses", async () => {
    ...
    const checkDetails: CheckDetails = {
      ...
      detailsUrl: "https://github.com/getrambla/rambla/actions/runs/456/job/789",
