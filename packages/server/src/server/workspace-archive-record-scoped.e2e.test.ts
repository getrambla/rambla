...
// derived from scope + last-reference + Rambla ownership; there is no caller-
// supplied disk flag.
...
test("archiving the last reference to a worktree removes it from disk regardless of the disk flag", async () => {
  ...
  const keepArchive = await ctx.client.archiveRamblaWorktree({ worktreePath: keepDir });
  ...
  const deleteArchive = await ctx.client.archiveRamblaWorktree({ worktreePath: deleteDir });
...
test("worktree archive targets the explicit workspaceId when a directory backs multiple workspaces", async () => {
  ...
  await ctx.client.archiveRamblaWorktree({
    worktreePath: worktreeDir,
