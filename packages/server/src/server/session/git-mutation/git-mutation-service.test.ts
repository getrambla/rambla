...
function initRepo(extraBranch?: string): string {
  ...
  run("config", "user.name", "Rambla Test");
...
function initClonedRepo(): string {
  ...
  run(seed, "config", "user.name", "Rambla Test");
  ...
  run(work, "config", "user.name", "Rambla Test");
  return work;
}
...
describe("createBranchFromBase", () => {
  ...
  // https://github.com/getrambla/rambla/issues/5213 — a branch that tracks origin/main sends the
  // next push to main: `git push` under push.default=tracking and Rambla's own push both read
  // branch.<name>.merge. A branch Rambla creates must carry no upstream, exactly as the worktree
