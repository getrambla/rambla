...
const cliPackagePath = path.join(globalNodeModules, "@getrambla", "cli");
...
describe("DefaultNpmGlobalRamblaCli", () => {
  test("inspects the npm global cli install with npm -g ls", async () => {
    ...
    expect(calls).toEqual([
      {
        command: "npm",
        args: ["-g", "ls", "@getrambla/cli", "--json", "--depth=0", "--long"],
        timeout: 10_000,
  ...
  test("runs the global install command for the latest cli", async () => {
    ...
    expect(calls).toEqual([
      {
        command: "npm",
        args: ["install", "-g", "@getrambla/cli@latest"],
        timeout: 300_000,
        maxBuffer: 10 * 1024 * 1024,
      },
    ]);
  ...
  test("reports missing global cli when npm output has no cli dependency", async () => {
    ...
    await expect(cli.inspect()).rejects.toThrow(
      "@getrambla/cli is not installed with npm -g on this host",
    );
  });
});
