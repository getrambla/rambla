...
import { RAMBLA_CLI_PACKAGE, type NpmGlobalRamblaInstall } from "./npm-global-cli.js";
...
export function validateDaemonInstallOrigin(
  install: NpmGlobalRamblaInstall,
  daemonVersion: string | null,
  runtime: DaemonInstallOriginRuntime = daemonInstallOriginRuntime,
): string | null {
  if (install.isLinked) {
    return `The global ${RAMBLA_CLI_PACKAGE} install is linked; self-update only supports normal npm global installs.`;
  }

  if (daemonVersion && install.version !== daemonVersion) {
    return `This daemon is not running from the npm global ${RAMBLA_CLI_PACKAGE} install (global npm has ${install.version}, daemon is ${daemonVersion}).`;
  }
  ...
  if (!isCurrentServerUnderNpmInstall(currentServerPackageRoot, install)) {
    return `This daemon is not running from the npm global ${RAMBLA_CLI_PACKAGE} install.`;
  }
