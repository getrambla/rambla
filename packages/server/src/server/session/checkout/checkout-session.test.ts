...
describe("CheckoutSession", () => {
  ...
  describe("stash list", () => {
    it("returns stash entries scoped to rambla stashes by default", async () => {
      const listStashesCalls: Array<{ cwd: string; ramblaOnly: boolean | undefined }> = [];
      const { checkout, emitted } = makeCheckoutSession({
        git: {
          listStashes: async (cwd, opts) => {
            listStashesCalls.push({ cwd, ramblaOnly: opts?.ramblaOnly });
            return [];
          },
        },
      });
      ...
      expect(listStashesCalls).toEqual([{ cwd: "/repo", ramblaOnly: true }]);
      expect(emitted).toEqual([
        {
