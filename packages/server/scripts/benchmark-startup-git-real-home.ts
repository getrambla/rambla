...
function parseArgs(): { sourceHome: string; frozenHomeRoot: string | null; scenario: Scenario } {
  let sourceHome = process.env.RAMBLA_BENCHMARK_SOURCE_HOME ?? path.join(os.homedir(), ".rambla");
  let frozenHomeRoot = process.env.RAMBLA_BENCHMARK_FROZEN_HOME_ROOT ?? null;
  let scenario = (process.env.RAMBLA_BENCHMARK_SCENARIO ?? "snapshotOnly") as Scenario;
