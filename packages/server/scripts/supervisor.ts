...
interface SupervisorHeartbeatMessage {
  type: "rambla:supervisor-heartbeat";
}

interface SupervisorGracefulShutdownMessage {
  type: "rambla:graceful-shutdown";
  reason: string;
}
...
function parseLifecycleMessage(msg: unknown): WorkerLifecycleMessage | null {
  ...
  if (type === "rambla:ready") {
    ...
    return { type: "rambla:ready", listen, serverId };
  }
  if (type === "rambla:restart") {
    const reason = (msg as { reason?: unknown }).reason;
    return {
      type: "rambla:restart",
      ...(typeof reason === "string" && reason.trim().length > 0 ? { reason } : {}),
...
export function runSupervisor(options: SupervisorOptions): SupervisorController {
  ...
  const spawnWorker = () => {
    ...
    const heartbeat = setInterval(() => {
      const message: SupervisorHeartbeatMessage = { type: "rambla:supervisor-heartbeat" };
    ...
    child.on("message", (msg: unknown) => {
      ...
      if (lifecycleMessage.type === "rambla:ready") {
        reachedReady = true;
      ...
      if (lifecycleMessage.type === "rambla:shutdown") {
