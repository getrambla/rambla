...
const LATE_IGNORED_DIRS = readPositiveInteger("RAMBLA_WATCH_BENCH_LATE_IGNORED_DIRS", 2_000);
const LATE_IGNORED_FILES = readPositiveInteger("RAMBLA_WATCH_BENCH_LATE_IGNORED_FILES", 20_000);
...
const CHURN_DIRECTORY_COUNT = readPositiveInteger("RAMBLA_WATCH_BENCH_CHURN_DIRECTORIES", 20_000);
const CHURN_FILES_PER_DIRECTORY = readPositiveInteger(
  "RAMBLA_WATCH_BENCH_CHURN_FILES_PER_DIRECTORY",
  3,
);
const CHURN_ROUNDS = readPositiveInteger("RAMBLA_WATCH_BENCH_CHURN_ROUNDS", 3);
const CHURN_DIRECTORIES_PER_ROUND = readPositiveInteger(
  "RAMBLA_WATCH_BENCH_CHURN_DIRECTORIES_PER_ROUND",
  500,
);
...
const CHURN_SETTLE_TIMEOUT_MS = readPositiveInteger(
  "RAMBLA_WATCH_BENCH_CHURN_SETTLE_TIMEOUT_MS",
  300_000,
);
...
function resolveMode(): BenchMode {
  const raw = process.env.RAMBLA_WATCH_BENCH_MODE;
...
async function measureDirectoryChurn(run: number): Promise<Measurement> {
  ...
  if (totalChurnTargets > CHURN_DIRECTORY_COUNT) {
    throw new Error(
      `RAMBLA_WATCH_BENCH_CHURN_ROUNDS (${CHURN_ROUNDS}) * ` +
        `RAMBLA_WATCH_BENCH_CHURN_DIRECTORIES_PER_ROUND (${CHURN_DIRECTORIES_PER_ROUND}) = ` +
        `${totalChurnTargets} must not exceed RAMBLA_WATCH_BENCH_CHURN_DIRECTORIES ` +
        `(${CHURN_DIRECTORY_COUNT})`,
    );
  }

  const base = await mkdtemp(join(tmpdir(), "rambla-watch-churn-"));
