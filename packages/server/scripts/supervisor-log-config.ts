...
export function resolveSupervisorLogFile(
  ...
  const envRotateSize = env.RAMBLA_LOG_ROTATE_SIZE?.trim();
  const envRotateMaxFiles = parseOptionalPositiveInteger(env.RAMBLA_LOG_ROTATE_COUNT);
  let logPath = path.join(ramblaHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (configuredPath) {
    logPath = path.isAbsolute(configuredPath)
      ? configuredPath
      : path.resolve(ramblaHome, configuredPath);
  }
