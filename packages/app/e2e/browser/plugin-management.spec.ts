...
>({
  pluginEnvironment: async ({ e2eWorker: _e2eWorker }, provide) => {
    const directory = await mkdtemp(path.join(tmpdir(), "rambla-plugins-e2e-"));
...
async function createGitPluginRepository(root: string): Promise<string> {
  ...
  execFileSync("git", ["config", "user.name", "Rambla Tests"], {
    cwd: repository,
    stdio: "ignore",
  });
  execFileSync("git", ["config", "user.email", "rambla@example.test"], {
    cwd: repository,
    stdio: "ignore",
  });
...
async function retryNpmInstallAndInspectRow(page: Page, width: number, testInfo: TestInfo) {
  ...
  await installPlugin(page, "npm:@rambla-fixture/review@^2.0.0");
  ...
  await expectSourceHierarchy(
    ...
    "npm:@rambla-fixture/review · 2.0.0",
  );
  await expect(
