...
interface RamblaOwnedWorktree {
  projectName: string;
  workspaceId: string;
  worktreeSlug: string;
}

async function withRamblaOwnedWorktree(
  run: (workspace: RamblaOwnedWorktree) => Promise<void>,
): Promise<void> {
  ...
  try {
    ...
    if (!created.workspace) {
      throw new Error(created.error ?? "Failed to create Rambla-owned worktree");
    }
...
test.describe("Sidebar workspace list", () => {
  ...
  test("Rambla-owned worktree hover card shows the worktree directory name", async ({ page }) => {
    await withRamblaOwnedWorktree(async ({ projectName, workspaceId, worktreeSlug }) => {
      await gotoAppShell(page);
      await waitForSidebarProject(page, projectName);
