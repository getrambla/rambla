...
async function openBrokenStartup(page: Page) {
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      "rambla:last-workspace-route-selection",
      JSON.stringify({ serverId: "fixture-host", workspaceId: "broken" }),
    );
  });
  await page.goto(appUrl);
  await expect(page.getByText("Rambla ran into a problem.", { exact: true })).toBeVisible();
}
async function readSavedWorkspaceState(page: Page) {
  return page.evaluate(() => ({
    layout: localStorage.getItem("workspace-layout-state"),
    selection: localStorage.getItem("rambla:last-workspace-route-selection"),
  }));
}
async function reloadToPicker(page: Page) {
  ...
  await expect(page.getByText("Rambla ran into a problem.", { exact: true })).toHaveCount(0);
}
