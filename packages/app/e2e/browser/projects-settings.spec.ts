...
import {
  blockRamblaConfigWrites,
  bumpRamblaConfigOnDisk,
...
const test = base.extend<ProjectsSettingsFixtures>({
  editableProject: async ({ page: _page }, provide) => {
    const workspace = await seedWorkspace({
      ...
      repo: { ramblaConfig: initialRamblaConfig },
    });
  ...
  gitlabRemoteProject: async ({ page: _page }, provide) => {
    const workspace = await seedWorkspace({
      ...
      repo: {
        ramblaConfig: initialRamblaConfig,
...
async function expectProjectConfigSaved(project: ProjectsSettingsProject): Promise<void> {
  await expect
    ...
    .toMatchObject({
      worktree: {
        setup: updatedSetup,
        teardown: initialRamblaConfig.worktree.teardown,
        customWorktreeField: initialRamblaConfig.worktree.customWorktreeField,
      },
      scripts: {
        dev: {
          command: initialRamblaConfig.scripts.dev.command,
          type: initialRamblaConfig.scripts.dev.type,
          port: initialRamblaConfig.scripts.dev.port,
          customScriptField: initialRamblaConfig.scripts.dev.customScriptField,
        },
      },
      customTopLevelField: initialRamblaConfig.customTopLevelField,
    });
...
async function readProjectConfigFile(project: ProjectsSettingsProject): Promise<string> {
  return readFile(path.join(project.path, "rambla.json"), "utf8");
}
...
test.describe("Projects settings", () => {
  ...
  test("user edits worktree setup from the projects page", async ({ page, editableProject }) => {
    ...
    commitRamblaConfig(editableProject.path);
...
test.describe("Projects settings — error UX", () => {
  test("stale-write callout appears on save, disables save, and reload clears it", async ({
    ...
    await bumpRamblaConfigOnDisk(editableProject.path);
  ...
  test("invalid rambla.json shows read-error callout, reload after fix shows form", async ({
    page,
    editableProject,
  }) => {
    await corruptRamblaConfig(editableProject.path);
    ...
    await restoreRamblaConfig(editableProject.path, initialRamblaConfig);
  ...
  test("write_failed callout appears on save with blocked directory, retry re-attempts, reload clears it", async ({
    ...
    await blockRamblaConfigWrites(editableProject.path);
    ...
    await unblockRamblaConfigWrites(editableProject.path);
    await clickReloadProjectSettings(page);
