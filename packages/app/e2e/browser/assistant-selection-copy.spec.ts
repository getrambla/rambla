...
const ASSISTANT_MARKDOWN = [
  ...
  "",
  "[workspace file](file:///tmp/rambla%20notes.md#L4)",
  "",
...
async function selectAssistantListItemFromMarker(
  ...
  const item = assistantMessage
    .locator(`[data-rambla-markdown-tag="${listTag}"]`)
    .filter({ hasText: itemText })
    .locator(':scope > [data-rambla-markdown-tag="li"]')
    .filter({ hasText: itemText });

  await item.evaluate((element, selectedEndText) => {
    const marker = element.querySelector(':scope > [data-rambla-markdown-list-marker="true"]');
...
async function doubleClickAssistantMarkdownText(
  ...
  await assistantMessage
    .locator(`[data-rambla-markdown-tag="${tag}"]`)
    .filter({ hasText: text })
...
test("copying an assistant selection preserves Markdown structure and links", async ({
  ...
  await page.addInitScript(() => {
    localStorage.setItem("@rambla:app-settings", JSON.stringify({ uiFontFamily: "serif" }));
  });
  ...
  try {
    ...
    const inlineCode = assistantMessage
      .locator('[data-rambla-markdown-tag="code"]')
    ...
    expect(clipboard.html).toContain(
      '<a href="file:///tmp/rambla%20notes.md#L4">workspace file</a>',
    );
    ...
    await selectAssistantElement(
      page,
      '[data-rambla-markdown-language="typescript"] [data-rambla-markdown-tag="code"]',
    );
    await copySelection(page);
    ...
    const bashFence = assistantMessage.locator('[data-rambla-markdown-language="bash"]');
    await bashFence.hover();
    await bashFence.locator("[data-rambla-markdown-ignore]").click();
