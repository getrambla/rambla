...
const RAMBLA_REPO_PATH = path.resolve(__dirname, "../../../..");
...
async function seedPaseoWorkspaceWithOpenCodeSession(): Promise<OpenCodeImportScenario> {
  ...
  await launchOpenCodeSessionInWorkspace(RAMBLA_REPO_PATH, prompt);
  const client = await connectSeedClient();
  try {
    const createdWorkspace = await client.createWorkspace({
      source: { kind: "directory", path: RAMBLA_REPO_PATH },
    });
    if (!createdWorkspace.workspace) {
      throw new Error(createdWorkspace.error ?? `Failed to create workspace ${RAMBLA_REPO_PATH}`);
    }
...
function formatOpenCodeLaunchError(result: OpenCodeSeedResult, prompt: string): string {
  return [
    ...
    `command: ${["opencode", ...openCodeSeedArgs(RAMBLA_REPO_PATH, prompt)].join(" ")}`,
