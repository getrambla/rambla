...
async function viewport(page: Page) {
  return page.evaluate(
    () =>
      (window as Window & { __ramblaTerminal?: { buffer: { active: { viewportY: number } } } })
        .__ramblaTerminal?.buffer.active.viewportY,
  );
}
...
async function readVimScreen(page: Page) {
  const rows = await page.evaluate(
    () => (window as Window & { __ramblaTerminal: { rows: number } }).__ramblaTerminal.rows,
  );
