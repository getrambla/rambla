...
const source = `import { useHosts, getRamblaClient, useRambla } from "@getrambla/plugin/client";
import { useState } from "react";
import { View, Text, Pressable } from "react-native";
export default function contribute(plugin) {
  function Surface() {
    const hosts = useHosts();
    const selected = useRambla();
    const [result, setResult] = useState("");
    const [subscriptionId, setSubscriptionId] = useState("");
    async function read(serverId) {
      setResult("Loading");
      try {
        const client = serverId ? getRamblaClient(serverId) : selected;
    ...
    async function disposeHost(serverId) {
      await getRamblaClient(serverId).dispose();
...
export async function installHostClientsScenario(page: Page) {
  const directory = await mkdtemp(path.join(tmpdir(), "rambla-plugin-hosts-"));
