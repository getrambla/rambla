...
async function defaultCodexModel(mcp: Client): Promise<string> {
  const result = z
    ...
    .parse(await callRamblaTool(mcp, "list_models", { provider: "codex" }));
  ...
  return {
    async createWorktreeWithCodex() {
      ...
      workspace = workspaceSchema.parse(
        await callRamblaTool(mcp, "create_workspace", {
          isolation: "worktree",
          path: repo.path,
          baseBranch: "main",
      ...
      const agent = z.object({ agentId: z.string() }).parse(
        await callRamblaTool(mcp, "create_agent", {
    ...
    async dispose() {
      ...
      try {
        ...
        await info.attach("daemon-log", {
          body: await readFile(path.join(process.env.E2E_RAMBLA_HOME!, "daemon.log")),
          contentType: "text/plain",
        });
      } catch (error) {
        errors.push(error);
      ...
      if (!testFailed || process.env.E2E_KEEP_RAMBLA_HOME !== "1") {
