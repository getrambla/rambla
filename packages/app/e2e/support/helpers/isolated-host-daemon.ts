...
export async function startIsolatedHostDaemon(
  ...
  const ramblaHome =
    options.ramblaHome ?? (await mkdtemp(path.join(tmpdir(), "rambla-e2e-secondary-host-")));
  let publishedPackageRoot: string | null = null;
  if (options.publishedVersion) {
    publishedPackageRoot = await mkdtemp(path.join(tmpdir(), "rambla-e2e-published-server-"));
    await writeFile(
    ...
    try {
      ...
      execFileSync(
        ...
        [
          ...
          "--no-fund",
          "--no-package-lock",
          `@getrambla/server@${options.publishedVersion}`,
        ],
  ...
  const serverDir = publishedPackageRoot
    ? path.join(publishedPackageRoot, "node_modules", "@getrambla", "server")
  ...
  const spawnDaemon = async (): Promise<ChildProcess> => {
    const spawnOptions: SpawnOptions = {
      ...
      env: withDisabledE2ESpeechEnv({
        ...
        RAMBLA_CORS_ORIGINS: `http://localhost:${metroPort}`,
        RAMBLA_RELAY_ENABLED: options.mutableRelay ? undefined : "0",
