...
export async function primeAdditionalPage(page: Page): Promise<void> {
  ...
  await page.addInitScript(
    ({ daemon: seededDaemon, preferences: seededPreferences, seedNonce: nonce }) => {
      ...
      localStorage.setItem("@rambla:daemon-registry", JSON.stringify([seededDaemon]));
      localStorage.removeItem("@rambla:settings");
      localStorage.setItem("@rambla:create-agent-preferences", JSON.stringify(seededPreferences));
    },
