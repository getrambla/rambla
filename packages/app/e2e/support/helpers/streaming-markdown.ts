  ...
  const agent = await seedMockAgentWorkspace({
    ...
    featureValues: {
      mockStreamingAssistantResponse:
        "**Bold text stays bold** and [Rambla docs](https://example.com/documentation). Done.",
...
export async function expectUnfinishedBold(page: Page): Promise<void> {
  ...
  const bold = message.locator('[data-rambla-markdown-tag="strong"]');
...
export async function expectUnfinishedLink(
  ...
  const message = page.getByTestId("assistant-message").last();
  await agent.stream.showThrough("**Bold text stays bold** and [Rambla docs");
  await expect(message).toContainText("Rambla docs");
  await expect(message.getByRole("link", { name: "Rambla docs" })).toHaveCount(0);
  await expect(message).not.toContainText("[");
...
async function expectCompletedMarkdown(page: Page): Promise<void> {
  const message = page.getByTestId("assistant-message").last();
  await expect(message).toHaveText("Bold text stays bold and Rambla docs. Done.");
  await expect(
    message.getByRole("link", { name: "Rambla docs" }).and(message.locator("a")),
  ).toHaveAttribute("href", "https://example.com/documentation");
  await expect(message.locator('[data-rambla-markdown-tag="strong"]')).toHaveCSS(
    "font-weight",
    "500",
  );
}
