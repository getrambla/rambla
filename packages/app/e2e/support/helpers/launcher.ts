...
export async function sampleTabsDuringTransition(
  ...
  await page.evaluate((duration) => {
    const scope = globalThis as typeof globalThis & {
      __ramblaTabTrackFrames?: Array<Array<{ id: string; width: number }>>;
    };
    scope.__ramblaTabTrackFrames = [];
    ...
    function sample() {
      ...
      scope.__ramblaTabTrackFrames?.push(
        tabs.map((element) => ({
  ...
  return page.evaluate(() => {
    ...
    return scope.__ramblaTabTrackFrames ?? [];
  });
}
