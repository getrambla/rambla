...
export interface RamblaHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveRamblaHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkRamblaHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<RamblaHomeMetadataForkResult> {
  const sourceHome = resolveRamblaHomePath(input.sourceHome);
  const targetHome = resolveRamblaHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Rambla metadata onto the same RAMBLA_HOME.");
  }
