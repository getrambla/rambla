...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@rambla.test"', { cwd: repoPath, stdio: "ignore" });
  ...
  if (options?.ramblaConfig) {
    await writeFile(
      path.join(repoPath, "rambla.json"),
      JSON.stringify(options.ramblaConfig, null, 2),
    );
  }
  ...
  if (options?.ramblaConfig) {
    execSync("git add rambla.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.rambla-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
 ...
 */
export async function createTempDirectory(prefix = "rambla-e2e-dir-"): Promise<TempDirectory> {
...
export async function trackForkUpstream(repoPath: string): Promise<string> {
  ...
  const upstreamClone = await mkdtemp(path.join(await resolveTempRoot(), "rambla-e2e-upstream-"));
  ...
  execSync('git config user.email "e2e@rambla.test"', { cwd: upstreamClone, stdio: "ignore" });
...
export async function readWorktreeBaseMetadata(
  ...
  const metadata = JSON.parse(
    await readFile(path.join(gitDir, "rambla", "worktree.json"), "utf8"),
