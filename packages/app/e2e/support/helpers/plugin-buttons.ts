...
import { useWorkspace, useRpc, useRambla } from "@getrambla/plugin/client";
...
function Details({ workspaceId, theme, layout, close }) {
  ...
  React.useEffect(() => {
    const owner = rambla.observeEvents(["project.update"]);
...
export default function contribute(client) {
  ...
  void client.rambla.workspaces.list({ subscribe: {}, signal: lifetime.signal }).then(({ subscription }) => {
