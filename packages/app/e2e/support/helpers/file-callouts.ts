...
export async function recordFileCallouts(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    Object.assign(globalThis, { __RAMBLA_RECORDED_FILE_CALLOUTS__: callouts });
  });
}
...
function recordedFileCallouts(page: Page): Promise<string[]> {
  return page.evaluate(
    () =>
      (globalThis as typeof globalThis & { __RAMBLA_RECORDED_FILE_CALLOUTS__?: string[] })
        .__RAMBLA_RECORDED_FILE_CALLOUTS__ ?? [],
  );
}
