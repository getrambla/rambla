...
export RAMBLA_MAESTRO_DAEMON_HEALTH_URL="${RAMBLA_MAESTRO_DAEMON_HEALTH_URL:-http://127.0.0.1:6767/api/health}"
...
if [ -z "${RAMBLA_MAESTRO_IOS_UDID:-}" ]; then
  export RAMBLA_MAESTRO_IOS_UDID
  RAMBLA_MAESTRO_IOS_UDID="$({ xcrun simctl list devices booted -j || true; } | node -e '
    let input = "";
...
render_flow() {
  ...
  local target="$2"
  perl -0pe '
    s/^appId: sh\.rambla$/appId: $ENV{RAMBLA_MAESTRO_APP_ID}/m;
    s/\$\{RAMBLA_MAESTRO_APP_ID\}/$ENV{RAMBLA_MAESTRO_APP_ID}/g;
    s/\$\{RAMBLA_MAESTRO_DIRECT_ENDPOINT\}/$ENV{RAMBLA_MAESTRO_DIRECT_ENDPOINT}/g;
    s/\$\{RAMBLA_MAESTRO_DRAG_A_NAME\}/$ENV{RAMBLA_MAESTRO_DRAG_A_NAME}/g;
    s/\$\{RAMBLA_MAESTRO_DRAG_B_NAME\}/$ENV{RAMBLA_MAESTRO_DRAG_B_NAME}/g;
    s/\$\{RAMBLA_MAESTRO_DRAG_Z_NAME\}/$ENV{RAMBLA_MAESTRO_DRAG_Z_NAME}/g;
  ' "$source" > "$target"
}

for project_name in \
  "$RAMBLA_MAESTRO_DRAG_A_NAME" \
  "$RAMBLA_MAESTRO_DRAG_B_NAME" \
  "$RAMBLA_MAESTRO_DRAG_Z_NAME"; do
...
const client = new DaemonClient({
  url: process.env.RAMBLA_MAESTRO_DAEMON_WS_URL,
...
const { DaemonClient } = await import(moduleUrl);
const projectNames = [
  process.env.RAMBLA_MAESTRO_DRAG_A_NAME,
  process.env.RAMBLA_MAESTRO_DRAG_B_NAME,
  process.env.RAMBLA_MAESTRO_DRAG_Z_NAME,
];
const client = new DaemonClient({
  url: process.env.RAMBLA_MAESTRO_DAEMON_WS_URL,
...
echo "Running sidebar drag cancellation regression on $RAMBLA_MAESTRO_IOS_UDID"
echo "Artifacts: $OUT_DIR"
(cd "$OUT_DIR" && maestro test "$FLOW" --udid "$RAMBLA_MAESTRO_IOS_UDID")
