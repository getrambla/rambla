...
#define METHOD(name) Java_sh_rambla_diffprototype_RamblaDiffPrototypeModule_##name
...
extern "C" JNIEXPORT jdoubleArray JNICALL METHOD(prepareDocument)(
  ...
  try {
    ...
    rambla::diff::prepare(*document, SkFontMgr_New_Android(nullptr), fontFamily.c_str(), size, job->cancelled);
...
extern "C" JNIEXPORT jdoubleArray JNICALL METHOD(readDocument)(
  ...
  try {
    ...
    return numbers(env, rambla::diff::read(*document, start, count));
  } catch (const std::exception& error) {
    fail(env, error);
    return nullptr;
  }
