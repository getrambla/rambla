...
class RamblaScrollPackage : ReactPackage, ViewManagerOnDemandReactPackage {
  ...
  override fun createViewManager(context: ReactApplicationContext, name: String): ViewManager<*, *>? =
    if (name == ReactScrollViewManager.REACT_CLASS) RamblaScrollViewManager() else null
}

private class RamblaScrollViewManager : ReactScrollViewManager() {
  override fun createViewInstance(context: ThemedReactContext): ReactScrollView =
    RamblaScrollView(context)
}

private class RamblaScrollView(context: ThemedReactContext) : ReactScrollView(context) {
