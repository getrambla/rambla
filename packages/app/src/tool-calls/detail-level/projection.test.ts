...
import type { ToolCallDetail } from "@getrambla/protocol/agent-types";
...
describe("tool call detail-level projection", () => {
  ...
  it("distinguishes reads, searches, and other tools in overview", () => {
    const calls = [
      ...
      toolCall(
        "4",
        { type: "search", query: "rambla", toolName: "web_search" },
        { status: "failed" },
      ),
  ...
  it("counts Rambla calls separately from other tools", () => {
    const calls = [
      toolCall("1", { type: "unknown", input: null, output: null }, { name: "rambla.list_agents" }),
      toolCall(
        "2",
        { type: "unknown", input: null, output: null },
        { name: "mcp__rambla__list_worktrees" },
      ),
      toolCall("3", { type: "fetch", url: "https://rambla.sh" }),
      toolCall("4", { type: "fetch", url: "https://github.com/getrambla" }),
    ];

    const result = project({ level: "overview", head: calls });

    expect(result.groupsByHostId.get("1")).toMatchObject({
      summary: { otherToolCount: 2, ramblaCallCount: 2 },
    });
  });
  ...
  it("classifies direct Brave search and Rambla runtime tool names", () => {
    ...
    const calls = [
      ...
      toolCall("3", unknownDetail, { name: "rambla_list_providers" }),
      toolCall("4", unknownDetail, { name: "rambla_list_worktrees" }),
      toolCall("5", unknownDetail, { name: "rambla_list_worktrees" }),
    ...
    expect(result.groupsByHostId.get("1")).toMatchObject({
      summary: { searchCount: 3, otherToolCount: 0, ramblaCallCount: 3 },
    });
  });
