    ...
    const projects = buildSidebarProjectsFromStructure({
      projects: [
        project({
          ...
          hosts: [
            {
              serverId: "relay:rambla-host",
              iconWorkingDir: "/repo/project-1",
              worktreeSupport: "supported" as const,
            },
          ],
          workspaceKeys: ["relay:rambla-host:ws-main"],
        }),
      ],
    });

    expect(projects[0]?.workspaces[0]).toMatchObject({
      workspaceKey: "relay:rambla-host:ws-main",
      serverId: "relay:rambla-host",
      workspaceId: "ws-main",
    });
  });
...
describe("shared sidebar workspace model", () => {
  it("feeds project placement and status grouping from the same cross-host workspace identities", () => {
    const model = buildSidebarWorkspacePlacementModel({
      projects: [
        project({
          projectKey: "getrambla/rambla",
          projectName: "getrambla/rambla",
          iconWorkingDir: "/repo/getrambla/rambla",
          hosts: [
            {
              serverId: "host-a",
              iconWorkingDir: "/repo/getrambla/rambla",
          ...
          workspaces: new Map([
            [
              ...
              workspace({
                id: "main",
                name: "main",
                projectId: "getrambla/rambla",
                projectDisplayName: "getrambla/rambla",
                status: "done",
              }),
            ],
          ]),
