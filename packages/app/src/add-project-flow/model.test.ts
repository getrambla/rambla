...
describe("Add Project options", () => {
  ...
  it("offers manual URL and protocol-specific owner/repo clone choices", () => {
    expect(buildManualGithubRepositoryChoices("git@github.com:getrambla/rambla.git")).toEqual([
      expect.objectContaining({
        id: "manual:git@github.com:getrambla/rambla.git",
        nameWithOwner: "getrambla/rambla",
        cloneUrl: "git@github.com:getrambla/rambla.git",
      }),
    ]);
    expect(buildManualGithubRepositoryChoices("getrambla/rambla")).toEqual([
      expect.objectContaining({ cloneProtocol: "https", cloneUrl: "getrambla/rambla" }),
      expect.objectContaining({ cloneProtocol: "ssh", cloneUrl: "getrambla/rambla" }),
    ]);
    expect(buildManualGithubRepositoryChoices("rambla")).toEqual([]);
  });
  ...
  it("shows final clone paths while retaining parent paths as values", () => {
    expect(
      buildCloneLocationOptions({
        ...
        repositoryName: "rambla",
        existingPaths: ["~/workspace/rambla"],
      }),
    ).toEqual([
      {
        id: "~/dev",
        path: "~/dev",
        displayPath: "~/dev/rambla",
