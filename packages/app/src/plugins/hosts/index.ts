...
import type { PluginHostSummary } from "@getrambla/plugin/client";
...
export function createPluginHosts(source: PluginHostsSource, signal: AbortSignal) {
  const clients = new Map<
    string,
    { client: DaemonClient; api: RamblaApi; lifetime: AbortController }
  >();
  ...
  function resolve(serverId: string): DaemonClient {
    ...
    if (!source.getHosts().some((host) => host.serverId === serverId)) {
      throw new Error(`Unknown Rambla host: ${serverId}`);
    }
    const host = source.getSnapshot(serverId);
    if (host?.connectionStatus !== "online" || !host.client) {
      throw new Error(`Rambla host is disconnected: ${serverId}`);
    }
    return host.client;
  }
  function refresh() {
  ...
  return {
    ...
    getRamblaClient(serverId: string): RamblaApi {
      ...
      const borrowed = new Proxy(client, {
        get(target, key) {
          ...
          return (...args: unknown[]) => {
            if (lifetime.signal.aborted) throw new Error(`Rambla client is released: ${serverId}`);
            if (resolve(serverId) !== client) {
              throw new Error(`Rambla connection changed; call getRamblaClient again: ${serverId}`);
            }
            return Reflect.apply(value, target, args);
          };
        },
      });
      const api = createRamblaApi(borrowed, { signal: lifetime.signal });
