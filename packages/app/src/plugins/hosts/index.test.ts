...
describe("plugin host access", () => {
  ...
  it("rejects unknown and disconnected targets without falling through to another host", () => {
    ...
    expect(() => h.runtime.getRamblaClient("missing")).toThrow("Unknown Rambla host: missing");
    expect(() => h.runtime.getRamblaClient("b")).toThrow("Rambla host is disconnected: b");
    h.lifetime.abort();
  });
  ...
  it("isolates installation lifetimes and releases APIs when a connection is replaced", async () => {
    ...
    const api = h.runtime.getRamblaClient("b");
    expect(api).toBe(h.runtime.getRamblaClient("b"));
    expect(api).not.toBe(other.getRamblaClient("b"));
    ...
    h.publish();
    expect(() => h.runtime.getRamblaClient("b")).toThrow("disconnected");
    expect(() => api.config.get()).toThrow("Rambla host is disconnected: b");
    h.snapshots.set("b", { connectionStatus: "online", client });
    h.publish();
    expect(h.runtime.getRamblaClient("b")).toBe(api);
    ...
    expect(() => api.config.get()).toThrow("Rambla client is released: b");
    expect(h.runtime.getRamblaClient("b")).not.toBe(api);
    h.lifetime.abort();
    expect(() => h.runtime.getRamblaClient("b")).toThrow("Plugin has stopped");
    expect(() => other.getRamblaClient("b").agents.subscribe(ignoreUpdate)).not.toThrow();
    otherLifetime.abort();
  });
});
...
it("reacquires a fresh API after explicit disposal without affecting a later borrower", async () => {
  ...
  const first = h.runtime.getRamblaClient("b");
  await first.dispose();
  const second = h.runtime.getRamblaClient("b");
  expect(second).not.toBe(first);
  expect(() => second.agents.subscribe(ignoreUpdate)).not.toThrow();
  expect(() => first.config.get()).toThrow("Rambla client is released: b");
  await first.dispose();
  expect(h.runtime.getRamblaClient("b")).toBe(second);
  h.lifetime.abort();
