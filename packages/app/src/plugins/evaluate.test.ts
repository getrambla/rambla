...
describe("evaluatePluginClientBundle", () => {
  ...
  it("keeps shared and client runtime exports separate", () => {
    expect(() =>
      ...
      const { ExternalLink } = require("@getrambla/plugin/client/ui");
      if (typeof ExternalLink !== "function") throw new Error("ExternalLink");
      for (const name of ["useRambla", "useRpc", "useSettings", "useAgent", "useWorkspace", "openExternalUrl"]) {
...
it("binds imported getters to each originating installation across delayed callbacks", async () => {
  ...
  const hostRuntime = (installation: string): PluginClientRuntime => ({
    ...
    hosts: {
      getSnapshot: () => [],
      subscribe: () => () => {},
      getRamblaClient(serverId) {
        calls.push(`${installation}/${serverId}`);
        return runtime.rambla;
      },
    },
  });
  const source = bundle(`
    const { getRamblaClient } = require("@getrambla/plugin/client");
    getRamblaClient("entry-host");
    plugin.addCommandCenterItem({
      id: "read", title: "Read", icon: "Server", context: "global",
      onSelect: async () => { await Promise.resolve(); getRamblaClient("target-host"); },
    });
  `);
