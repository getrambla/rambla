...
import { CheckoutPrStatusSchema } from "@getrambla/protocol/messages";
...
describe("git-actions-policy", () => {
  ...
  it("keeps push available for a no-upstream Rambla worktree with local commits", () => {
    const actions = buildGitActions(
      createInput({
        hasRemote: true,
  ...
  it("offers archive workspace for Git checkouts and worktrees", () => {
    ...
    const worktree = buildGitActions(
      createInput({ hasUncommittedChanges: true, isRamblaOwnedWorktree: true }),
    );
  ...
  it("still promotes archive as primary for an idle Rambla-owned worktree", () => {
    const actions = buildGitActions(createInput({ isRamblaOwnedWorktree: true }));
