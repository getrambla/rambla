...
function message(messageId: string, ...blocks: string[]) {
  ...
  for (const block of blocks) {
    ...
    row.innerHTML = `<div data-message-text="true"><div data-rambla-markdown-tag="p">${block}</div></div>`;
    root.append(row);
  }
...
it("collects a message's occurrences across its block rows in row order", () => {
  ...
  other.innerHTML =
    '<div data-message-text="true"><div data-rambla-markdown-tag="p">alpha three</div></div>';
  root.append(other);
...
it("finds each actual occurrence across inline formatting with original Unicode offsets", () => {
  const row = content(
    '<div data-rambla-markdown-tag="p">İ😀hello <strong>world</strong> then <em>hello</em> world</div>',
  );
...
it("ignores controls, keeps block boundaries, and preserves literal code punctuation", () => {
  const row = content(
    '<div data-rambla-markdown-tag="p">first</div><div data-rambla-markdown-tag="p">second</div><div data-rambla-markdown-tag="pre"><span>const a</span><span>.b</span><button>copy a.b</button><span data-rambla-markdown-ignore="true">a.b</span></div>',
  );
...
it("matches whitespace across text nodes without inspecting hidden link destinations", () => {
  const row = content(
    '<div data-rambla-markdown-tag="p"><a href="https://example.com/hidden">hello &amp; world</a>\nline</div>',
  );
