...
function transcript(messageId: string, blocks: string[]): HTMLElement {
  ...
  for (const [index, text] of blocks.entries()) {
    ...
    row.innerHTML = `<div data-message-text="true"><div data-rambla-markdown-tag="p">${text}</div></div>`;
    root.append(row);
  }
...
function viewport(root: HTMLElement, capture: Capture) {
  ...
  return createFindViewport({
    ...
    getRoot: () => root,
    highlightName: "rambla-chat-find-test",
  });
}

afterEach(() => {
  CSS.highlights.delete("rambla-chat-find-test");
...
function highlightedRanges(): Range[] {
  const highlight = CSS.highlights.get("rambla-chat-find-test");
