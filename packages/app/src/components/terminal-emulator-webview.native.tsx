...
export default function WebViewTerminalEmulator({
  ...
  const flushPendingMessages = useCallback(() => {
    ...
    for (const message of pending) {
      ...
      webViewRef.current.injectJavaScript(
        `window.__RAMBLA_TERMINAL_WEBVIEW_RECEIVE__ && window.__RAMBLA_TERMINAL_WEBVIEW_RECEIVE__(${payload}); true;`,
      );
    }
  }, []);
  ...
  useImperativeHandle(
    ...
    (): TerminalEmulatorHandle => ({
      ...
      blur: () => {
        webViewRef.current?.injectJavaScript(
          "window.__RAMBLA_TERMINAL_WEBVIEW_BLUR__ && window.__RAMBLA_TERMINAL_WEBVIEW_BLUR__(); true;",
        );
        Keyboard.dismiss();
      },
    }),
