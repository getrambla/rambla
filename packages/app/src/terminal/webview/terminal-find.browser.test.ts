...
interface TerminalFrameWindow extends Window {
  __RAMBLA_TERMINAL_WEBVIEW_RECEIVE__(message: unknown): void;
  __ramblaTerminal?: Terminal;
}
...
test("the generated WebView searches its mounted stream and keeps Find commands off the input bridge", async () => {
  ...
  const send = (message: object) =>
    win.__RAMBLA_TERMINAL_WEBVIEW_RECEIVE__({ streamKey: "owned", ...message });
  send({
    type: "mount",
  ...
  await expect
    .poll(() => {
      const b = win.__ramblaTerminal?.buffer.active;
      return b
  ...
  expect(win.__ramblaTerminal!.getSelection()).toBe("a.b");
  send({ type: "clearFind" });
  await expect
    .poll(() => messages.findLast((m) => m.type === "findResult")?.result)
