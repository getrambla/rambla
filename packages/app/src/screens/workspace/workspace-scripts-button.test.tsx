...
const {
  ...
  const routePreferenceByServerId: Record<string, "public" | "rambla" | "direct"> = {};
  const routePreferenceListeners = new Set<() => void>();
  const setPreferredRoute = vi.fn((serverId: string, kind: "public" | "rambla" | "direct") => {
  ...
  it("uses service icon color for service health and running unknown status only", () => {
    current = renderScripts([
      ...
      script({
        scriptName: "worker",
        type: "service",
        hostname: "worker.rambla.localhost",
        lifecycle: "running",
        health: null,
        port: 5000,
      }),
      script({
        ...
        type: "service",
        hostname: "old-service.rambla.localhost",
        lifecycle: "stopped",
        exitCode: 1,
      }),
    ]);
