...
import type { AgentTimelineItem, ToolCallDetail } from "@getrambla/protocol/agent-types";
import {
  createUserMessage,
  hydrateStreamState,
...
describe("processAgentStreamEvents", () => {
  ...
  it("keeps Claude image tool-result output before following assistant blocks while text streams", () => {
    const imageMarkdown =
      "![Image](/tmp/rambla-attachments/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.png)";
    const result = processAgentStreamEvents({
      events: [
        makeStreamReducerEvent(
          {
