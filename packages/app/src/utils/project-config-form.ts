...
export interface ProjectConfigDraft {
  ...
  metadataGenerationBase: RamblaMetadataGeneration | undefined;
}
...
export function configToDraft(config: RamblaConfigRaw | null | undefined): ProjectConfigDraft {
...
interface ApplyDraftInput {
  ...
  base: RamblaConfigRaw | null | undefined;
}

export function applyDraftToConfig(input: ApplyDraftInput): RamblaConfigRaw {
  ...
  const nextScripts: Record<string, RamblaScriptEntryRaw> = {};
  for (const row of input.draft.scripts) {
    ...
    nextScripts[trimmedName] = nextEntry as RamblaScriptEntryRaw;
  }
  ...
  return result as RamblaConfigRaw;
}
