...
function makeHost(): HostProfile {
  return {
    ...
    connections: [
      ...
      {
        id: "socket:/tmp/rambla-secret.sock",
        type: "directSocket",
        path: "/tmp/rambla-secret.sock",
      },
      {
        id: "pipe:\\\\.\\pipe\\rambla-secret",
        type: "directPipe",
        path: "\\\\.\\pipe\\rambla-secret",
      },
    ],
  };
}
...
describe("app diagnostics report", () => {
  test("reports whether the connected daemon is managed by Rambla Desktop", () => {
  ...
  test("formats connection rows without raw connection details", () => {
    ...
    expect(report).not.toContain("/tmp/rambla-secret.sock");
  ...
  test("redacts saved connection secrets from collected daemon and desktop text", () => {
    ...
    expect(redacted).not.toContain("/tmp/rambla-secret.sock");
    expect(redacted).not.toContain("\\\\.\\pipe\\rambla-secret");
