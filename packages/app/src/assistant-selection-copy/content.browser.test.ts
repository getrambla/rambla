...
const fixture = `
  <div data-testid="assistant-message">
    <div data-rambla-markdown-tag="p">Prefix <span data-rambla-markdown-tag="strong">bold text</span> and <span data-rambla-markdown-tag="code">inline code</span> suffix.</div>
    <div data-rambla-markdown-tag="ul">
      <div data-rambla-markdown-tag="li"><span data-rambla-markdown-ignore="true" data-rambla-markdown-list-marker="true">•</span><div><span>First bullet text</span></div></div>
      <div data-rambla-markdown-tag="li"><span data-rambla-markdown-ignore="true" data-rambla-markdown-list-marker="true">•</span><div><span>Second bullet text</span></div></div>
    </div>
    <div data-rambla-markdown-tag="pre" data-rambla-markdown-language="ts"><span data-rambla-markdown-tag="code">const answer = true;</span></div>
  </div>
`;
...
function mountTranscript(messages: TranscriptMessage[]): HTMLElement {
  ...
  for (const { messageId, blocks } of messages) {
    for (const [index, text] of blocks.entries()) {
      ...
      row.innerHTML = `<div data-testid="assistant-message"><div data-rambla-markdown-tag="p">${text}</div></div>`;
...
describe("assistant selection copy ranges", () => {
  it("does not handle absent, collapsed, or non-assistant selections", () => {
    ...
    const strong = fixtureElement(message, '[data-rambla-markdown-tag="strong"]');
  ...
  it("copies a selection that runs across the block rows of one message", () => {
    ...
    const blocks = transcript.querySelectorAll('[data-rambla-markdown-tag="p"]');
  ...
  it("does not assemble a message group from rows outside a transcript", () => {
    ...
    transcript.remove();
    const blocks = document.querySelectorAll('[data-rambla-markdown-tag="p"]');
  ...
  it("does not replace the browser clipboard for a range spanning assistant messages", () => {
    ...
    const firstText = fixtureElement(firstMessage, '[data-rambla-markdown-tag="strong"]');
    const secondText = fixtureElement(secondMessage, '[data-rambla-markdown-tag="strong"]');
  ...
  it.each([
    ...
    ({ tag, range, expected, forbiddenHtml }) => {
      const message = mountFixture();
      const element = fixtureElement(message, `[data-rambla-markdown-tag="${tag}"]`);
  ...
  it.each(["strong", "em", "s", "h2", "blockquote"])(
    ...
    (tag) => {
      const message = mountFixture();
      const element = fixtureElement(message, '[data-rambla-markdown-tag="strong"]');
      element.setAttribute("data-rambla-markdown-tag", tag);
      const content = createAssistantSelectionClipboardContent(
  ...
  it("copies complete inline code without delimiters when the selection stays inside", () => {
    const message = mountFixture();
    const element = fixtureElement(message, '[data-rambla-markdown-tag="code"]');
    const content = createAssistantSelectionClipboardContent(
      selectText(element, 0, textNode(element).length),
    );
  ...
  it("keeps a complete inline node but drops formatting from a partial node at the other edge", () => {
    const message = mountFixture();
    const strong = fixtureElement(message, '[data-rambla-markdown-tag="strong"]');
    const code = fixtureElement(message, '[data-rambla-markdown-tag="code"]');
  ...
  it("copies list-item text without inventing a bullet", () => {
    const message = mountFixture();
    const itemText = fixtureElement(message, '[data-rambla-markdown-tag="li"] div span');
    const content = createAssistantSelectionClipboardContent(
  ...
  it("retains a bullet when the range includes the marker and the complete item", () => {
    const message = mountFixture();
    const item = fixtureElement(message, '[data-rambla-markdown-tag="li"]');
  ...
  it("retains a bullet when a drag selects from the rendered marker through the item text", () => {
    const message = mountFixture();
    const marker = fixtureElement(message, '[data-rambla-markdown-list-marker="true"]');
    const itemText = fixtureElement(message, '[data-rambla-markdown-tag="li"] div span');
  ...
  it("retains every selected marker across a partial multi-item drag", () => {
    const message = mountFixture();
    const markerSelector = '[data-rambla-markdown-list-marker="true"]';
    const textSelector = '[data-rambla-markdown-tag="li"] div span';
  ...
  it("retains the original number when a marker drag starts mid-list", () => {
    const message = mountFixture();
    const list = fixtureElement(message, '[data-rambla-markdown-tag="ul"]');
    list.setAttribute("data-rambla-markdown-tag", "ol");
    list.setAttribute("data-rambla-markdown-list-start", "5");
    const markers = list.querySelectorAll<HTMLElement>('[data-rambla-markdown-list-marker="true"]');
    markers.item(0).textContent = "5.";
    markers.item(1).textContent = "6.";
    const secondMarker = fixtureElement(list, '[data-rambla-markdown-list-marker="true"]', 1);
    const secondText = fixtureElement(list, '[data-rambla-markdown-tag="li"] div span', 1);
  ...
  it("retains nested markers when a drag includes the complete outer item", () => {
    const message = mountFixture();
    const firstItem = fixtureElement(message, '[data-rambla-markdown-tag="li"]');
    firstItem.innerHTML = [
      '<span data-rambla-markdown-ignore="true" data-rambla-markdown-list-marker="true">•</span>',
      "<div>",
      "<span>Outer text</span>",
      '<div data-rambla-markdown-tag="ul">',
      '<div data-rambla-markdown-tag="li">',
      '<span data-rambla-markdown-ignore="true" data-rambla-markdown-list-marker="true">•</span>',
    ...
    const outerMarker = fixtureElement(
      firstItem,
      ':scope > [data-rambla-markdown-list-marker="true"]',
    );
    const innerText = fixtureElement(
      firstItem,
      ':scope [data-rambla-markdown-tag="ul"] > [data-rambla-markdown-tag="li"] > div > span',
    );
  ...
  it("preserves paragraph breaks when rich HTML flattens a loose list item", () => {
    ...
    item.insertAdjacentHTML(
      "beforeend",
      '<div data-rambla-markdown-tag="p">First paragraph</div><div data-rambla-markdown-tag="p">Second paragraph</div>',
    );
  ...
  it("omits a partial leading bullet and retains the marker crossed before the trailing item", () => {
    const message = mountFixture();
    const selector = '[data-rambla-markdown-tag="li"] div span';
  ...
  it("copies part of a fenced code block without adding an inline or fenced delimiter", () => {
    ...
    const blockCode = fixtureElement(
      message,
      '[data-rambla-markdown-tag="pre"] [data-rambla-markdown-tag="code"]',
    );
...
function highlightedFixture(language: string | null): string {
  const languageAttribute =
    language === null ? "" : ` data-rambla-markdown-language="${escapeAttribute(language)}"`;
  return [
    '<div data-testid="assistant-message">',
    `<div data-rambla-markdown-tag="pre"${languageAttribute}>`,
    '<span data-rambla-markdown-tag="code">',
    ...
    "<span>    doThing();</span>",
    "</span>",
    '<div data-rambla-markdown-ignore="true"><span>Copy</span></div>',
    "</div>",
    '<div data-rambla-markdown-tag="p"><span>After the block.</span></div>',
    "</div>",
  ].join("");
}
