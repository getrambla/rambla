...
async function resolveRunWorkspace(
  ...
  const ambientWorkspaceId = newWorkspace ? undefined : process.env.RAMBLA_WORKSPACE_ID?.trim();
  ...
  console.error(
    "Tip: pass --workspace <id> (or set RAMBLA_WORKSPACE_ID) to run in an existing workspace.",
  );
...
// RAMBLA_AGENT_ID names an agent on the daemon that launched this shell. A run
...
export async function resolveRunCallerAgentId(
  ...
  env: { RAMBLA_AGENT_ID?: string } = process.env,
): Promise<string | undefined> {
  const agentId = env.RAMBLA_AGENT_ID?.trim();
  if (!agentId) {
    return undefined;
  }
