...
export async function runArchiveCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: rambla agent archive <id-or-name>",
    };
    throw error;
  }
  ...
  try {
    ...
    if (!agentId) {
      const error: CommandError = {
        ...
        details: 'Use "rambla ls" to list available agents',
      };
      throw error;
    }
    ...
    if (agent.status === "running" && !options.force) {
      const error: CommandError = {
        ...
        details:
          "Use --force to archive a running agent (it will interrupt the active run), or stop it first with: rambla agent stop. Use rambla agent delete to hard-delete it.",
      };
      throw error;
    }
