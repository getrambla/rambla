...
describe("parseScheduleCreateInput cwd/host validation", () => {
  ...
  test("RAMBLA_HOST without cwd → throws MISSING_CWD", () => {
    const daemonTarget = selectDaemonTarget({}, { RAMBLA_HOST: "dev:12345" });
  ...
  test("explicit home beats RAMBLA_HOST and defaults cwd locally", () => {
    const daemonTarget = selectDaemonTarget({ home: process.cwd() }, { RAMBLA_HOST: "dev:12345" });
