...
export async function resolveLocalPairingOffer(options: {
  ...
  const instance = await readDaemonInstance(options.ramblaHome);
  if (instance)
    return resolveDaemonPairingOffer(
      { kind: "instance", home: options.ramblaHome },
      options.enableRelay,
    );
  if (options.enableRelay)
    editPersistedConfig(options.ramblaHome, "daemon.relay.enabled", { value: true });
  const config = resolveConfigFromPersisted(
    options.ramblaHome,
    readPersistedConfig(options.ramblaHome, { defaultsIfMissing: true }),
    { env: {} },
  );
...
export async function runPairCommand(options: PairOptions): Promise<void> {
  ...
  const resolveOffer = (enableRelay: boolean) =>
    target.kind === "instance"
      ? resolveLocalPairingOffer({ ramblaHome: target.home, enableRelay })
  ...
  if (offline)
    output.writeStderr(
      `Offline pairing offer. Start with: rambla daemon start --home ${JSON.stringify(target.kind === "instance" ? target.home : "")}\n`,
    );
