...
async function fixture() {
  const root = await mkdtemp(path.join(tmpdir(), "rambla lifecycle "));
  const env = {
    ...Object.fromEntries(
...
test("managed two-home restart retains its supervisor and never routes ordinary commands or stop to the other home", async () => {
  ...
  try {
    ...
    const poisoned = {
      RAMBLA_HOME: a,
      RAMBLA_HOST: `127.0.0.1:${portA}`,
      RAMBLA_LISTEN: `127.0.0.1:${portA}`,
      PORT: String(portA),
    ...
    if (process.platform !== "win32") {
      for (const home of [a, b, path.join(f.root, ".rambla")])
...
test("status reports the server id of a password-protected daemon to a caller without the password", async () => {
  ...
  try {
    ...
    const authenticated = await f.liveStatus(home, { RAMBLA_PASSWORD: "secret" });
...
test("worker restart preserves an already-running legacy supervisor's launch flags", async () => {
  ...
  try {
    ...
    const launch = await startDaemonInstance({
      ...
      // The legacy CLI translated --port into RAMBLA_LISTEN before spawning.
      env: { ...f.env, RAMBLA_LISTEN: `127.0.0.1:${launchPort}` },
      mode: "deployment",
      timeoutMs: 30_000,
    });
    ...
    const restarted = await f.ok(["restart", "--home", home, "--timeout", "30"], {
      RAMBLA_LISTEN: `127.0.0.1:${filePort}`,
    });
...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    ...
    try {
      ...
      await f.ok(["start", "--home", a, "--timeout", "30"]);
      await f.ok(["start", "--home", b, "--timeout", "30"], { RAMBLA_SERVER_ID: "live-b" });
      const beforeA = await f.liveStatus(a);
      expect((await f.liveStatus(b)).serverId).toBe("live-b");
      await f.ok(["restart", "--home", b, "--timeout", "30"]);
      const bLock = JSON.parse(await readFile(path.join(b, "rambla.pid"), "utf8"));
      await writeFile(
        path.join(b, "rambla.pid"),
...
test("empty explicit selectors never select the ambient daemon or create local state", async () => {
  ...
  try {
    ...
    for (const args of [
      ...
      const refused = await f.run(["--json", ...args], { RAMBLA_HOME: home });
      expect(refused.code).toBe(1);
      expect(refused.stderr).toContain("TARGET_INVALID");
      expect((await f.liveStatus(home)).workerPid).toBe(before.workerPid);
      expect(existsSync(path.join(f.root, "rambla.pid"))).toBe(false);
...
test("raw log following subscribes to a stored agent that exists only in B", async () => {
  ...
  try {
    ...
    follower = spawn(
      ...
      {
        cwd: f.root,
        env: { ...f.env, RAMBLA_HOME: a, RAMBLA_HOST: launchA.listen },
        stdio: ["ignore", "pipe", "pipe"],
      },
    );
    let output = "",
...
test("foreground deployment retains environment until its owner ends the launch; endpoint stop reports acknowledgment", async () => {
  ...
  try {
    ...
    deployment = spawn(process.execPath, [cli, "daemon", "run", "--home", home], {
      cwd: f.root,
      env: { ...f.env, RAMBLA_LISTEN: deploymentEndpoint, RAMBLA_RELAY_ENABLED: "false" },
      stdio: "ignore",
    });
    await expect
    ...
    const restarted = await f.ok(["restart", "--home", home], { RAMBLA_LISTEN: fileEndpoint });
