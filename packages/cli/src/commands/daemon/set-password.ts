...
function terminalPasswordPrompt(): PromptPassword {
  if (!process.stdin.isTTY) {
    throw createCommandError(
      ...
      "rambla daemon set-password needs a terminal to read the password",
      "Run it in an interactive terminal, or set RAMBLA_PASSWORD in the daemon's environment instead.",
    );
  }
...
export async function setDaemonPasswordInConfig(
  ...
  savePersistedConfig(ramblaHome, nextConfig);
