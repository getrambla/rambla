...
import { startDaemonInstance, resolveRamblaHome } from "@getrambla/server/daemon-control";
...
export async function launchLocalDaemon(options: {
  ...
  try {
    ...
    return await startDaemonInstance({
      home: resolveRamblaHome({ RAMBLA_HOME: options.home }),
  ...
  command.hook("preAction", () => {
    for (const [flag, configPath] of Object.entries(REMOVED_LAUNCH_FLAGS)) {
      ...
      throw {
        ...
        message: `${flag.split(" ")[0]} was removed. ${configPath ? `Use rambla daemon config set ${configPath} <value> --home <path>, then start or restart.` : "Use rambla daemon run --home <path> for foreground deployment."} Deployment environment overrides belong to rambla daemon run.`,
      };
    }
  });
  return command;
