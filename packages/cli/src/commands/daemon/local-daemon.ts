...
import { loadConfig, resolveRamblaHome, spawnProcess } from "@getrambla/server";
...
function envWithHome(home?: string): NodeJS.ProcessEnv {
  ...
  return { ...process.env, RAMBLA_HOME: home };
}
...
function buildChildEnv(options: DaemonStartOptions): NodeJS.ProcessEnv {
  ...
  if (options.home) {
    childEnv.RAMBLA_HOME = options.home;
  }
  if (options.listen) {
    childEnv.RAMBLA_LISTEN = options.listen;
  } else if (options.port) {
    childEnv.RAMBLA_LISTEN = `127.0.0.1:${options.port}`;
  }
  if (options.hostnames) {
    childEnv.RAMBLA_HOSTNAMES = options.hostnames;
  }
  if (options.relayUseTls === true) {
    childEnv.RAMBLA_RELAY_USE_TLS = "true";
  }
  if (options.webUi === true) {
    childEnv.RAMBLA_WEB_UI_ENABLED = "true";
  }
  if (options.webUi === false) {
    childEnv.RAMBLA_WEB_UI_ENABLED = "false";
  }
  return childEnv;
}
...
function resolveServerRunnerFromDir(currentDir: string): string | null {
  ...
  try {
    ...
    if (packageJson.name !== "@getrambla/server") return null;
...
function resolveDaemonRunnerEntry(): string {
  const serverExportPath = require.resolve("@getrambla/server");
  ...
  throw new Error("Unable to resolve @getrambla/server package root for daemon runner");
}

function pidFilePath(ramblaHome: string): string {
  return path.join(ramblaHome, DAEMON_PID_FILENAME);
}
...
export function resolveLocalDaemonState(options: { home?: string } = {}): LocalDaemonState {
  const env: NodeJS.ProcessEnv = {
    ...
    RAMBLA_LISTEN: undefined,
    RAMBLA_HOSTNAMES: undefined,
    RAMBLA_ALLOWED_HOSTS: undefined,
    RAMBLA_RELAY_ENABLED: undefined,
    RAMBLA_RELAY_ENDPOINT: undefined,
    RAMBLA_RELAY_PUBLIC_ENDPOINT: undefined,
...
export async function startLocalDaemonDetached(
  ...
  const ramblaHome = runtime.resolveHome(childEnv);
  const logPath = path.join(ramblaHome, DAEMON_LOG_FILENAME);
