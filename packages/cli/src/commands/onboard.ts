...
import { readDaemonInstance, waitForDaemonReady } from "@getrambla/server/daemon-control";
...
function printNextSteps(pairingUrl: string | null, ramblaHome: string, richUi: boolean): void {
  ...
  const nextStepsLines = [
    ...
    `5. Example: rambla run --home ${JSON.stringify(ramblaHome)} --output-schema schema.json "extract fields"`,
  ];
  const quickReferenceLines = [
    "1. rambla --help",
    `2. rambla ls --home ${JSON.stringify(ramblaHome)}`,
    `3. rambla run --home ${JSON.stringify(ramblaHome)} "your prompt"`,
    `4. rambla status --home ${JSON.stringify(ramblaHome)}`,
...
function persistSetupChoices(ramblaHome: string, options: OnboardOptions): void {
  const persisted = loadPersistedConfig(ramblaHome, { defaultsIfMissing: true });
...
export async function runOnboard(options: OnboardOptions): Promise<void> {
  ...
  const ramblaHome = options.daemonTarget.home;
  const alreadyRunning = await readDaemonInstance(ramblaHome);
  persistSetupChoices(ramblaHome, options);
  if (richUi) {
  ...
  if (alreadyRunning) {
    ...
    await launchLocalDaemon({ home: ramblaHome, timeoutMs });
  }
  const ready = await waitForDaemonReady(ramblaHome, { timeoutMs });
