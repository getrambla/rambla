...
describe("runCli", () => {
  it("defaults an empty CLI invocation to onboard", () => {
    expect(
      createCliParseArgv({
        argv: [],
        cwd: process.cwd(),
        nodeArgv: ["node", "rambla"],
      }),
    ).toEqual(["node", "rambla", "onboard"]);
  });
  ...
  it("routes explicit root relay flags to onboard", () => {
    ...
    ).toEqual(["node", "rambla", "onboard", "--relay"]);
    expect(
      createCliParseArgv({
        argv: ["--no-relay"],
        cwd: process.cwd(),
    ...
    ).toEqual(["node", "rambla", "onboard", "--no-relay"]);
  });
  ...
  it("preserves known CLI command argv", () => {
    ...
    ).toEqual(["node", "rambla", "daemon", "set-password"]);
  });
  ...
  it("preserves the hooks command argv", () => {
    ...
    ).toEqual(["node", "rambla", "hooks", "claude", "UserPromptSubmit"]);
  });

  it("classifies existing unknown directories as open-project invocations", () => {
    const root = mkdtempSync(path.join(tmpdir(), "rambla-cli-run-"));
