...
async function runDelete(args: string[]) {
  return $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npm --prefix ${repoRoot} run cli -- delete ${args}`.nothrow();
}
...
try {
...

  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npm --prefix ${repoRoot} run cli -- -q delete abc123`.nothrow();
    const output = result.stdout + result.stderr;
