...
const ramblaHome = await mkdtemp(join(tmpdir(), "rambla-stop-tree-kill-"));
const childPidPath = join(ramblaHome, "descendant.pid");
...
try {
  ...
  const stopResult =
    await $`RAMBLA_HOME=${ramblaHome} RAMBLA_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.RAMBLA_LOCAL_SPEECH_AUTO_DOWNLOAD} RAMBLA_DICTATION_ENABLED=${testEnv.RAMBLA_DICTATION_ENABLED} RAMBLA_VOICE_MODE_ENABLED=${testEnv.RAMBLA_VOICE_MODE_ENABLED} npx rambla daemon stop --home ${ramblaHome} --json --timeout 1 --force --kill-timeout 2`.nothrow();
