...
const root = await mkdtemp(join(tmpdir(), "rambla-set-password-"));
const ramblaHome = join(root, ".rambla");
...
try {
  {
    ...
    const result = await setDaemonPasswordInConfig("shared-secret", { home: ramblaHome });
...

  {
    ...
    const run = runLocalRambla(["daemon", "set-password", "--home", pipedHome, "--json"]);
    run.stdin.end("x\nx\n");
