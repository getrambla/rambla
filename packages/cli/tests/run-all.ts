...
/**
 * Test runner for Rambla CLI E2E tests
 *
 ...
 * its own daemon (ephemeral port + tmp RAMBLA_HOME), so parallelism is safe.
 */
...
const concurrencyEnv = process.env.RAMBLA_CLI_TEST_CONCURRENCY;
...
const shardTotal = parsePositiveInt(process.env.RAMBLA_CLI_TEST_SHARD_TOTAL, 1);
const shardIndexRaw = parsePositiveInt(process.env.RAMBLA_CLI_TEST_SHARD, 1);
if (shardIndexRaw < 1 || shardIndexRaw > shardTotal) {
  throw new Error(
    `RAMBLA_CLI_TEST_SHARD=${shardIndexRaw} out of range for SHARD_TOTAL=${shardTotal}`,
  );
}
...
console.log("🧪 Rambla CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
async function runSingleTest(testFile: string): Promise<TestOutcome> {
  ...
  const osHome = await mkdtemp(join(tmpdir(), "rambla-cli-test-os-home-"));
    ...
    return await new Promise<TestOutcome>((resolve) => {
      const proc = spawn("npx", ["tsx", testPath], {
        env: {
          ...
          RAMBLA_LOCAL_SPEECH_AUTO_DOWNLOAD: testEnvDefaults.RAMBLA_LOCAL_SPEECH_AUTO_DOWNLOAD,
          RAMBLA_DICTATION_ENABLED: testEnvDefaults.RAMBLA_DICTATION_ENABLED,
          RAMBLA_VOICE_MODE_ENABLED: testEnvDefaults.RAMBLA_VOICE_MODE_ENABLED,
        },
