...
try {
  ...
  // Test 3: permit ls returns error when no daemon running
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo permit ls`.nothrow();
  ...
  // Test 4: permit ls --json handles errors
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo permit ls --json`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted globally
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo -q permit ls`.nothrow();
    const output = result.stdout + result.stderr;
