...
try {
  ...
  // Test 3: permit ls returns error when no daemon running
  {
    ...
    const result =
      await $`RAMBLA_HOME=${ramblaHome} npx rambla --host localhost:${port} permit ls`.nothrow();
  ...
  // Test 4: permit ls --json handles errors
  {
    ...
    const result =
      await $`RAMBLA_HOME=${ramblaHome} npx rambla --host localhost:${port} permit ls --json`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted globally
  {
    ...
    const result =
      await $`RAMBLA_HOME=${ramblaHome} npx rambla --host localhost:${port} -q permit ls`.nothrow();
    const output = result.stdout + result.stderr;
