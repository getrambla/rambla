 ...
 * 2. Create agent with `rambla run "say hello" --provider claude`
 * 3. List agents with `rambla ls`
 * 4. Wait for agent with `rambla wait <id>`
 * 5. Inspect agent with `rambla inspect <id>`
 * 6. Stop agent with `rambla stop <id>` and verify it remains inspectable
 * 7. Delete agent with `rambla delete <id>`
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`RAMBLA_HOME: ${ctx.ramblaHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.rambla(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.rambla(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.rambla(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.rambla(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.rambla(["stop", agentId]);
  ...
  const inspectResult = await ctx.rambla(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.rambla(["delete", agentId]);
