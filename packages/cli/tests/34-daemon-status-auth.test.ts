...
if (process.platform !== "win32") {
  const root = await mkdtemp(join(tmpdir(), "rambla status slow provider "));
...
`,
    ...
    const local = await runLocalRambla(["daemon", "status", "--home", home, "--json"], {
      RAMBLA_PASSWORD: "shared-secret",
    });
    ...
    const remote = await runLocalRambla(
