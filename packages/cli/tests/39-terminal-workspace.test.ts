...
const sdk = createRamblaClient({ url: `${ctx.wsUrl}/ws`, reconnect: { enabled: false } });

async function cli(args: string[]) {
  const result = await ctx.rambla([...args, "--json"]);
...
try {
  ...
  const invalid = await ctx.rambla([
    "terminal",
    "create",
    "--workspace",
    "wks_missing",
  ...
  const conflict = await ctx.rambla(["terminal", "ls", "--all", "--workspace", second.id]);
  ...
  await waitForTerminalOutput(ctx.rambla, terminal.id, "READY");
  assert.deepEqual(await cli(["terminal", "send-keys", terminal.id, "-l", "Enter"]), {
    terminalId: terminal.id,
    keysSent: 5,
  });
  await waitForTerminalOutput(ctx.rambla, terminal.id, "HEX:456e746572");
  assert.deepEqual(await cli(["terminal", "send-keys", terminal.id, "Enter"]), {
    terminalId: terminal.id,
    keysSent: 1,
  });
  await waitForTerminalOutput(ctx.rambla, terminal.id, "HEX:456e7465720d");
