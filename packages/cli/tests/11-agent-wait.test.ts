...
try {
  ...
  // Test 2: wait requires id argument
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo wait`.nothrow();
  ...
  // Test 3: wait handles daemon not running
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo wait abc123`.nothrow();
  ...
  // Test 4: wait --timeout flag is accepted
  {
    ...
    const result =
      await $`RAMBLA_HOME=${paseoHome} npx paseo wait --timeout 30 --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: wait --host flag is accepted
  {
    ...
    const result =
      await $`RAMBLA_HOME=${paseoHome} npx paseo wait --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: -q (quiet) flag is accepted with wait
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo -q wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --json flag is accepted with wait
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo wait abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: --format yaml flag is accepted with wait
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo --format yaml wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
