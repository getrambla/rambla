...
try {
  ...
  // Test 2: inspect requires id argument
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo inspect`.nothrow();
  ...
  // Test 3: inspect handles daemon not running
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo inspect abc123`.nothrow();
  ...
  // Test 4: inspect --host flag is accepted
  {
    ...
    const result =
      await $`RAMBLA_HOME=${paseoHome} npx paseo inspect --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: -q (quiet) flag is accepted with inspect
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo -q inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: --json flag is accepted with inspect
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo inspect abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --format yaml flag is accepted with inspect
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo --format yaml inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
