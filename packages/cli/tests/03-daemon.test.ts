...
try {
  ...
  {
    const result = await runLocalRambla(["--host", "127.0.0.1:1", "daemon", "status", "--json"], {
  ...
  // Test 8: status uses the running daemon as relay authority over local IPC
  {
    ...
    try {
      ...
      assert.deepStrictEqual(JSON.parse(nestedReload.stdout), {
        restartCommand: `rambla daemon restart --host ${JSON.stringify(listen)}`,
      ...
      try {
        ...
        const foreignPairing = await runLocalRambla(
