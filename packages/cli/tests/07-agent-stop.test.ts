...
try {
  ...
  // Test 2: stop requires ID, --all, or --cwd
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo stop`.nothrow();
  ...
  // Test 3: stop handles daemon not running
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo stop abc123`.nothrow();
  ...
  // Test 4: stop --all flag is accepted
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo stop --all`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: stop --cwd flag is accepted
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo stop --cwd /tmp`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: stop with ID and --host flag is accepted
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo stop abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: -q (quiet) flag is accepted with stop
  {
    ...
    const result =
      await $`RAMBLA_HOST=localhost:${port} RAMBLA_HOME=${paseoHome} npx paseo -q stop abc123`.nothrow();
    const output = result.stdout + result.stderr;
