...
const ramblaHome = await mkdtemp(join(tmpdir(), "rambla-restart-supervisor-"));
...
const availabilityLog = join(ramblaHome, "availability.log");

try {
  if (process.platform !== "win32") {
    const provider = join(ramblaHome, "slow-provider");
    await writeFile(
      provider,
      `#!${process.execPath}
...
`,
  ...
  let supervisor = await readDaemonInstance(ramblaHome);
  await waitFor(
    async () => {
      supervisor = await readDaemonInstance(ramblaHome);
  ...
  const current = await readDaemonInstance(ramblaHome);
  ...
  await runLocalRambla(["daemon", "stop", "--home", ramblaHome, "--force"]);
