...
try {
  {
    ...
    const created = await ctx.rambla(
      [
        "schedule",
        "create",
        "Review new PRs",
    ...
    const listed = await ctx.rambla(["schedule", "ls", "--json"]);
    ...
    const inspected = await ctx.rambla(["schedule", "inspect", createdJson.id, "--json"]);
  ...
  await ctx.stop();
  await rm(ctx.ramblaHome, { recursive: true, force: true });
