...
describe("plugin selector shallow equality", () => {
  it("keeps equivalent selector objects and arrays stable", () => {
    ...
    expect(shallowEqual({ name: "Rambla", shared }, { name: "Rambla", shared })).toBe(true);
    expect(shallowEqual(["Rambla", shared], ["Rambla", shared])).toBe(true);
    expect(shallowEqual({ name: "Rambla" }, { name: "Changed" })).toBe(false);
  });
