...
export RAMBLA_ELECTRON_FLAGS="$(node -e 'process.stdout.write(JSON.parse(process.argv[1]).electronFlags)' "$DEV_RUNTIME")"
export RAMBLA_ELECTRON_USER_DATA_DIR="$(node -e 'process.stdout.write(JSON.parse(process.argv[1]).userDataDir)' "$DEV_RUNTIME")"
unset RAMBLA_DEV_RUNTIME_FALLBACK_ROOT
mkdir -p "$RAMBLA_ELECTRON_USER_DATA_DIR"
...
export RAMBLA_DAEMON_ENDPOINT="$DAEMON_ENDPOINT"
...
echo "  Daemon:     ${RAMBLA_LISTEN}"
echo "  Home:       ${RAMBLA_HOME}"
echo "  userData:   ${RAMBLA_ELECTRON_USER_DATA_DIR}"
echo "══════════════════════════════════════════════════════"
