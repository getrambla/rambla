...
const CDP_PORT = process.env.RAMBLA_ELECTRON_REMOTE_DEBUGGING_PORT ?? "9223";
...
async function readBridgeFullscreen(page) {
  return page.evaluate(
    async () =>
      (await window.ramblaDesktop?.window?.getCurrentWindow?.()?.isFullscreen?.()) === true,
  );
}

async function setNativeFullscreen(page, fullscreen) {
  await page.evaluate(async (nextFullscreen) => {
    const win = window.ramblaDesktop?.window?.getCurrentWindow?.();
...
async function inspectFullscreenWindowChrome(page, platform) {
  ...
  try {
    ...
    const details = await page.evaluate(async () => {
      const bridge = window.ramblaDesktop?.window?.getCurrentWindow?.();
...
async function detectDesktopBridge(page) {
  return page.evaluate(() => {
    const bridge = window.ramblaDesktop;
...
async function main() {
  ...
  try {
    ...
    const desktopStatus = await page.evaluate(() =>
      window.ramblaDesktop.invoke("desktop_daemon_status"),
    );
    assert(
