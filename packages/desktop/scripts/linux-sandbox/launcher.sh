...
for arg in "$@"; do
  if [ "$arg" = '--no-sandbox' ]; then
    export RAMBLA_DESKTOP_SANDBOX_REASON='requested by --no-sandbox'
    printf '[linux-sandbox] disabled: %s\n' "$RAMBLA_DESKTOP_SANDBOX_REASON" >&2 || true
    exec "$executable" "$@"
  fi
done
...
set -f
for flag in ${RAMBLA_ELECTRON_FLAGS:-}; do
  if [ "$flag" = '--no-sandbox' ]; then
    export RAMBLA_DESKTOP_SANDBOX_REASON='requested by RAMBLA_ELECTRON_FLAGS'
    printf '[linux-sandbox] disabled: %s\n' "$RAMBLA_DESKTOP_SANDBOX_REASON" >&2 || true
...
if namespace_error=$(unshare --user --map-root-user --net true 2>&1); then
  export RAMBLA_DESKTOP_SANDBOX_REASON='user namespaces available'
...
if [ -x "$helper" ] && [ "$(stat -Lc '%u:%a' -- "$helper")" = '0:4755' ] &&
  ...
  case ",$mount_options," in
    *,nosuid,*|,,) ;;
    *)
      export RAMBLA_DESKTOP_SANDBOX_REASON='root-owned SUID helper available'
      printf '[linux-sandbox] enabled: %s\n' "$RAMBLA_DESKTOP_SANDBOX_REASON" >&2 || true
      exec "$executable" "$@"
      ;;
  esac
...
export RAMBLA_DESKTOP_SANDBOX_REASON="user namespaces unavailable${namespace_error:+ ($namespace_error)}; no usable SUID helper"
printf '[linux-sandbox] disabled: %s\n' "$RAMBLA_DESKTOP_SANDBOX_REASON" >&2 || true
exec "$executable" --no-sandbox "$@"
