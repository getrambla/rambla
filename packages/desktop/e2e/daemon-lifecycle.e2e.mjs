...
import { savePersistedConfig } from "@getrambla/server/configuration";
import {
  startDaemonInstance,
  stopDaemonInstance,
  readDaemonInstance,
...
const root = await mkdtemp(path.join(tmpdir(), "rambla desktop lifecycle "));
const home = path.join(root, "daemon");
const env = Object.fromEntries(
  Object.entries(process.env).filter(
    ([key]) => !key.startsWith("RAMBLA_") && key !== "ELECTRON_RUN_AS_NODE",
  ),
);
env.HOME = root;
env.USERPROFILE = root;
env.RAMBLA_HOME = home;
...
async function openDesktop() {
  desktop = await electron.launch({
    ...
    env: {
      ...env,
      RAMBLA_LISTEN: "127.0.0.1:1",
      RAMBLA_WEB_UI_ENABLED: "false",
      RAMBLA_HOST: "unused:1",
    },
  });
  await expect
...
try {
  ...
  const artifacts =
    process.env.RAMBLA_DESKTOP_LIFECYCLE_ARTIFACT_DIR ??
    (await mkdtemp(path.join(tmpdir(), "rambla-desktop-lifecycle-artifacts-")));
