...
export function seedPluginLinks(ramblaHome, workspaceId, url, remoteWorkspaceId) {
  const directory = path.join(ramblaHome, "link-plugin");
  fs.mkdirSync(directory, { recursive: true });
  fs.writeFileSync(
    path.join(directory, "rambla-plugin.json"),
    JSON.stringify({ id: "link-check", requirements: { rambla: ">=0.8.0" } }),
  );
  fs.writeFileSync(
...
export async function runPluginLinksRegression({
  ...
  await page.evaluate((port) => {
    const key = "@rambla:daemon-registry";
