...
declare global {
  interface Window {
    __ramblaStartupPresentationTrace?: StartupPresentation[];
  }
}
...
async function observeStartupPresentation(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    window.__ramblaStartupPresentationTrace = trace;
...
async function getStartupPresentation(page: Page): Promise<StartupPresentation[]> {
  return page.evaluate(() => window.__ramblaStartupPresentationTrace?.slice() ?? []);
}
