...
async function clickGuestElement(page, client, browserId, selector) {
  ...
  await page.locator(`[data-rambla-browser-id="${browserId}"]`).click({
    position: {
...
async function main() {
  ...
  try {
    ...
    const daemon = spawnLogged(
      ...
      { cwd: rootDir, env: { ...commonEnv, RAMBLA_NODE_ENV: "development" } },
      artifactDir,
    );
    ...
    const desktop = spawnLogged(
      ...
      desktopCommandArgs,
      {
        ...
        env: {
          ...
          RAMBLA_ELECTRON_REMOTE_DEBUGGING_PORT: String(cdpPort),
