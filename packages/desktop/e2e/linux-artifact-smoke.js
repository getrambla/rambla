...
async function main() {
  ...
  const artifactRoot = process.env.RAMBLA_DESKTOP_SMOKE_ARTIFACT_DIR;
  const installedOnly = process.argv.includes("--installed-only");
  const extracted = fs.mkdtempSync(path.join(os.tmpdir(), "rambla-linux-artifacts-"));
  ...
  try {
    process.env.RAMBLA_DESKTOP_SMOKE_ARTIFACT_DIR = path.join(artifactRoot, "installed");
    const helper = fs.statSync("/opt/Rambla/chrome-sandbox");
    ...
    await smokePackagedDesktopApp({ appPath: "/opt/Rambla", expectedSandbox: true });
    ...
    const desktopEntry = fs.readFileSync(path.join(appDir, "Rambla.desktop"), "utf8");
    if (/^Exec=.*--no-sandbox/m.test(desktopEntry)) {
      throw new Error("AppImage desktop entry bypasses runtime sandbox policy");
    }
    process.env.RAMBLA_DESKTOP_SMOKE_ARTIFACT_DIR = path.join(artifactRoot, "appimage");
    ...
    const appPath = fs.existsSync(path.join(tarDir, "Rambla"))
      ? tarDir
      : path.join(tarDir, fs.readdirSync(tarDir)[0]);
    process.env.RAMBLA_DESKTOP_SMOKE_ARTIFACT_DIR = path.join(artifactRoot, "tar");
