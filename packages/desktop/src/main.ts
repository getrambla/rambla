...
if (process.platform === "linux") {
  ...
  app.setDesktopName("Rambla.desktop");
  if (!app.commandLine.hasSwitch("class")) app.commandLine.appendSwitch("class", "Rambla");
  log.info("[linux-sandbox]", {
    enabled: !app.commandLine.hasSwitch("no-sandbox"),
    reason: process.env.RAMBLA_DESKTOP_SANDBOX_REASON ?? "Chromium default",
  });
}
