...
export function registerWindowManager(input: { mode: DesktopWindowChromeMode }): void {
  ipcMain.handle("rambla:window:minimize", (event) => {
    BrowserWindow.fromWebContents(event.sender)?.minimize();
  });

  ipcMain.handle("rambla:window:close", (event) => {
    BrowserWindow.fromWebContents(event.sender)?.close();
  });

  ipcMain.handle("rambla:window:toggleMaximize", (event) => {
  ...
  ipcMain.handle("rambla:window:isFullscreen", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    return win?.isFullScreen() ?? false;
  });

  ipcMain.handle("rambla:window:isMaximized", (event) => {
    return BrowserWindow.fromWebContents(event.sender)?.isMaximized() ?? false;
  });

  ipcMain.handle("rambla:window:setFullscreen", (event, fullscreen: unknown) => {
    if (typeof fullscreen !== "boolean") return;
    BrowserWindow.fromWebContents(event.sender)?.setFullScreen(fullscreen);
  });

  ipcMain.handle("rambla:window:setBadgeCount", (_event, count?: unknown) => {
  ...
  ipcMain.handle("rambla:window:updateChrome", (event, update?: unknown) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) {
      return;
    }
...
export function setupWindowResizeEvents(win: BrowserWindow): void {
  ...
  const notifyResized = () => {
    ...
    win.webContents.send("rambla:window:resized", {});
  };
