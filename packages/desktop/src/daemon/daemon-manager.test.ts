...
vi.mock("electron", () => ({
  app: {
    getPath: vi.fn(() => mocks.ramblaHome),
...
vi.mock("./runtime-paths.js", () => ({
  ...
  resolveDaemonRunnerEntrypoint: vi.fn(() => ({
    entryPath: path.join(mocks.ramblaHome, "daemon.js"),
    execArgv: [],
  })),
}));
...
describe("daemon-manager commands", () => {
  ...
  beforeEach(() => {
    fixtureRoot = mkdtempSync(path.join(tmpdir(), "rambla daemon manager "));
    mocks.ramblaHome = path.join(fixtureRoot, "home");
