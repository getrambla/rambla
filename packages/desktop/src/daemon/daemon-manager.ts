...
export function isDesktopManagedDaemonRunningSync(): boolean {
  ...
  try {
    const lock = JSON.parse(readFileSync(path.join(ownedLaunch.home, "rambla.pid"), "utf8"));
...
async function startDaemon(): Promise<DesktopDaemonStatus> {
  ...
  try {
    await startDaemonInstance({
      ...
      timeoutMs: 30_000,
      ...invocation,
      env: { ...invocation.env, RAMBLA_CLI: getBundledCliShimPath() },
      mode: "managed",
      desktopManaged: true,
      onAcquired: (instance) => {
...
async function restartDaemon(): Promise<DesktopDaemonStatus> {
  await runExternalCliJsonCommand(["daemon", "restart", "--home", getRamblaHome(), "--json"]);
  return resolveDesktopDaemonStatus();
}
...
async function getCliDaemonStatus(): Promise<string> {
  return await runExternalCliTextCommand(["daemon", "status", "--home", getRamblaHome()]);
}
  ...
  return {
    ...
    desktop_sandbox_diagnostics: () =>
      describeSandbox({
        ...
        launcherReason: process.env.RAMBLA_DESKTOP_SANDBOX_REASON,
      }),
