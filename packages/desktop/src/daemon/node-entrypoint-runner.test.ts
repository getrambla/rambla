...
declare global {
  ...
  var __RAMBLA_NODE_ENTRYPOINT_RUNNER_FIXTURE__:
    | {
        argv: string[];
...
describe("node-entrypoint-runner", () => {
  it("preserves Electron node-mode env before loading the target entrypoint", async () => {
    ...
    writeFileSync(
      fixturePath,
      `
globalThis.__RAMBLA_NODE_ENTRYPOINT_RUNNER_FIXTURE__ = {
  argv: process.argv,
...
`,
    ...
    delete globalThis.__RAMBLA_NODE_ENTRYPOINT_RUNNER_FIXTURE__;

    try {
      await main();

      expect(globalThis.__RAMBLA_NODE_ENTRYPOINT_RUNNER_FIXTURE__).toEqual({
      ...
      globalThis.__RAMBLA_NODE_ENTRYPOINT_RUNNER_FIXTURE__ = originalFixture;
