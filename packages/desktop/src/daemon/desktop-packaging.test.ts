...
function createFakeMacBundle(options: { includeHelper: boolean }): {
  ...
  const root = mkdtempSync(join(tmpdir(), "rambla-cli-shim-test-"));
  const appPath = join(root, "Rambla.app");
  const contentsPath = join(appPath, "Contents");
  const resourcesPath = join(contentsPath, "Resources");
  const shimPath = join(resourcesPath, "bin", "rambla");
  ...
  if (options.includeHelper) {
    ...
    writeExecutable(
      ...
      [
        "#!/bin/sh",
        'printf "helper env=%s/%s cli=%s\\n" "$ELECTRON_RUN_AS_NODE" "$RAMBLA_NODE_ENV" "$RAMBLA_CLI"',
        'printf "args=%s\\n" "$*"',
        "",
      ].join("\n"),
    );
...
describe("desktop packaging", () => {
  ...
  it("unpacks server zsh shell integration files for external shells", () => {
    ...
    expect(config).toContain(
      "node_modules/@getrambla/server/dist/server/terminal/shell-integration/**/*",
    );
    expect(config).not.toContain(
      "node_modules/@getrambla/server/dist/src/terminal/shell-integration/**/*",
    );
  });
  ...
  it("excludes package debug/source files from the packaged app", () => {
    ...
    expect(config).toContain("!node_modules/@getrambla/*/src/**");
    expect(config).toContain("!node_modules/@getrambla/**/*.test.*");
    expect(config).toContain("!node_modules/@getrambla/**/*.spec.*");
  });

  it("excludes the bundled daemon web UI from the packaged app", () => {
    const config = readFileSync(join(packageRoot, "electron-builder.yml"), "utf8");

    expect(config).toContain("!node_modules/@getrambla/server/dist/server/web-ui/**");
  });
  ...
  it("registers Rambla agent links with the operating system", () => {
    ...
    expect(config).toContain("- rambla");
  });
  ...
  it("declares all workspace packages required at runtime", () => {
    ...
    for (const required of ["@getrambla/cli", "@getrambla/server"]) {
  ...
  it("launches the packaged macOS CLI through Helper instead of the main app executable", () => {
    ...
    try {
      ...
      expect(result.stdout).toContain("@getrambla/cli/dist/index.js");
