...
async function launch(
  ...
  const root = mkdtempSync(join(tmpdir(), "rambla-launcher-"));
  try {
    ...
    chmodSync(join(app, "Rambla"), 0o755);
    ...
    const executablePath = options.symlink ? join(root, "rambla") : join(app, "Rambla");
    if (options.symlink) symlinkSync(join(app, "Rambla"), executablePath);
    ...
    const result = spawnSync(executablePath, args, {
      ...
      env: {
        ...
        APPIMAGE: "/tmp/Rambla.AppImage",
        RAMBLA_DESKTOP_SMOKE: "0",
        ...options.env,
      },
    });
...
it("applies a debugging environment sandbox override before Chromium starts", async () => {
  const result = await launch({
    namespaces: true,
    env: { RAMBLA_ELECTRON_FLAGS: "--disable-gpu\t--no-sandbox" },
  });
  expect(result.args).toEqual(["--no-sandbox", ...result.input]);
  expect(result.stderr).toContain("requested by RAMBLA_ELECTRON_FLAGS");
});
