 ...
 *
 * Output: pretty table to stdout + JSON to /tmp/rambla-terminal-bench/<ts>.json
 *
...
async function bootDaemon(): Promise<BootedDaemon> {
  ...
  const ramblaHome = await mkdtemp(path.join(os.tmpdir(), "rambla-bench-home-"));
  ...
  return { child, port, ramblaHome, pid: child.pid ?? -1 };
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(`Daemon ready: pid=${daemon.pid} port=${daemon.port} home=${daemon.ramblaHome}`);

    const ctor = await loadDaemonClientCtor();
    client = await connectClient(ctor, daemon.port);

    workspaceDir = await mkdtemp(path.join(os.tmpdir(), "rambla-bench-ws-"));
    ...
    const outDir = "/tmp/rambla-terminal-bench";
    ...
    if (daemon) {
      ...
      await rm(daemon.ramblaHome, { recursive: true, force: true }).catch(() => undefined);
    }
    if (workspaceDir) {
