...
const timeoutMs = Number(args["timeout-ms"] ?? process.env.RAMBLA_PROVE_TIMEOUT_MS ?? 60_000);
const stabilityMs = Number(args["stability-ms"] ?? process.env.RAMBLA_PROVE_STABILITY_MS ?? 30_000);

if (!serverId || typeof serverId !== "string") {
  console.error("Missing server ID. Provide --server-id or RAMBLA_SERVER_ID.");
  process.exit(2);
}
if (!daemonPublicKeyB64 || typeof daemonPublicKeyB64 !== "string") {
  console.error(
    "Missing daemon public key. Provide --daemon-public-key-b64 or RAMBLA_DAEMON_PUBLIC_KEY_B64.",
  );
  process.exit(2);
}
