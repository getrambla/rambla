...
From the repository root, `npm run typecheck --workspace=@getrambla/plugin` checks this example
against the SDK. Existing plugin projects must update `@getrambla/plugin` before running their own
...
```sh
npm run test:e2e --workspace=@getrambla/app -- e2e/browser/plugin-button-example.spec.ts
```
