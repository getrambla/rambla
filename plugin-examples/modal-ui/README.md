...
Install on a Rambla host with npm source installation support and npm available:

```sh
rambla plugin install npm:@getrambla/plugin-example-modal-ui@0.1.1
```
...
installed version. With reviewed-update support, run `rambla plugin update modal-ui-example --check`
to check for a newer version, then `rambla plugin update modal-ui-example` to review and approve it.
An explicit install version selects the initial content; later updates still check the latest release.

The package ships the existing TypeScript entry and client sources. Rambla compiles them through
its directory plugin loader; no package build or preparation command is required. React,
React Native, and `@getrambla/plugin` are peer contracts supplied by the host. npm is needed for
installation, not loading or reloading. You can also install this directory while developing.
See the [source reference](https://rambla.sh/docs/plugins/reference#plugin-sources)
...
[host UI reference](https://rambla.sh/docs/plugins/reference#host-ui) for the API contract.
...
Run the [native sheet regression](https://github.com/getrambla/rambla/blob/main/packages/app/e2e/mobile/modal-sheet/README.md) to check body
