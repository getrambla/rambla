---
title: Connectivity
description: Connect a Rambla client to your daemon through SSH, the relay, or Tailscale.
nav: Connectivity
order: 4
...
Your Rambla app connects to the daemon running on your computer or server. Rambla Desktop and the CLI can tunnel through SSH. Mobile clients can connect through the Rambla relay or directly with Tailscale.
...
SSH transport connects to an existing daemon through your local OpenSSH client. It does not install, start, or configure Rambla on the remote host.

Before connecting:

1. Start the Rambla daemon on the remote host.
2. Confirm `ssh user@host` works with a key or SSH agent. Rambla uses non-interactive SSH and follows your OpenSSH config.
...
### Enable relay from Rambla Desktop

1. Open **Settings → your host → Pair a device**.
2. Select **Enable relay**.
3. Scan the QR code with Rambla on your phone, or copy the pairing link and paste it into the phone app.
...
2. Open Rambla and go to **Settings → Add host → Direct connection**.
3. Enter the Tailscale IP in **Host**.
4. Enter `6767` in **Port**.
5. Leave **Use SSL** off and select **Connect**.

If the host was already paired through the relay, Rambla adds the direct connection to the same host.
