...
When creating an agent, open the model picker and select a saved profile under **Profiles**. Rambla applies its settings together; you can still adjust them before sending your prompt.
...
The **When to use** field is the profile's notes. Rambla exposes these notes through its tools so an orchestrating agent can read them before choosing how to launch a worker.
...
After [enabling Rambla tools](/docs/orchestration#get-started), ask:

> Check my Rambla profiles and their notes. Choose one for the settings-page UI change, then use a review profile for an independent review of the diff.
