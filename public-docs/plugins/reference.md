...
The required root manifest is `rambla-plugin.json`:
...
Use `ExternalLink` to open documentation outside Rambla:

```tsx
import { ExternalLink } from "@getrambla/plugin/client/ui";

export function DocumentationLink() {
  return <ExternalLink href="https://rambla.sh/docs">Open documentation</ExternalLink>;
}
```
...
```ts
import { openExternalUrl } from "@getrambla/plugin/client";

export async function openDocumentation() {
  await openExternalUrl("https://rambla.sh/docs");
}
```
...
```tsx
import type { PluginWorkspacePanelProps } from "@getrambla/plugin/client";
import { ExternalLink } from "@getrambla/plugin/client/ui";
...
export function DocumentationPanel({ navigation, workspaceId, theme }: PluginWorkspacePanelProps) {
  ...
  const url = "https://rambla.sh/docs";
...
paragraph separators. Rambla invokes transformers before splitting native Markdown or grouping
...
The [local plugin example](https://github.com/getrambla/rambla/blob/main/plugin-examples/local-plugin/client/main.tsx)
...
Use `useHosts()` to display configured hosts and `getRamblaClient(serverId)` in an action callback
to run SDK operations on one of them:

```tsx
import { getRamblaClient, useHosts, type PluginSurfaceProps } from "@getrambla/plugin/client";
...
export function HostAgents({ theme }: Pick<PluginSurfaceProps, "theme">): ReactElement {
  ...
  async function listAgents(serverId: string): Promise<void> {
    try {
      const { entries } = await getRamblaClient(serverId).agents.list();
...
| ------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `serverId`    | `string`                                                     | ID to pass to `getRamblaClient`.                             |
...
`getRamblaClient(serverId: string): RamblaApi` borrows the host's authenticated app connection.
...
| ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unknown host ID                                                                          | Throws `Unknown Rambla host: <id>`; never falls through to another host.                                                                                        |
| Host is not online                                                                       | Throws `Rambla host is disconnected: <id>`, including calls through a retained API. Retry when online.                                                          |
| Same connection reconnects                                                               | Retained APIs remain usable after reconnection; observations resume automatically.                                                                              |
| Connection settings change or the app switches connections, including automatic failover | The old API is released. Call `getRamblaClient(serverId)` again and recreate subscriptions.                                                                     |
...
| Surface host selection changes                                                           | `useRambla()` follows the selected host. An explicitly acquired API keeps its original target.                                                                  |
...
[host agents example](https://github.com/getrambla/rambla/tree/main/plugin-examples/hosts).
...
```bash
rambla plugin install /srv/rambla/plugins/review
rambla plugin install github:acme/rambla-review
rambla plugin install git:https://git.example.com:8443/acme/monorepo.git:plugins/review --ref main
rambla plugin install git@git.example.com:acme/review.git
rambla plugin install file:///srv/repos/monorepo:plugins/review
rambla plugin install npm:rambla-review@1.2.0
rambla plugin install npm:@acme/rambla-review@next
rambla plugin install 'npm:@acme/rambla-review@>=1.2.0 <2.0.0' --id review-staging
rambla plugin install npm:@acme/plugins@^1.2.0:plugins/review
```

`--ref` applies only to Git and accepts a branch, tag, or commit for this installation. Without it,
Rambla installs the remote's default HEAD. Installation selectors do not constrain later updates. The legacy
...
rambla plugin update --all --yes
rambla plugin update my-plugin --version 1.2.0
rambla plugin update my-plugin --ref v2
...
> **Trust every plugin you add.** `rambla plugin add` and `rambla plugin install` mean “I trust this codebase.” Server code and preparation commands run unsandboxed with the daemon user's access on the daemon host; client contributions run inside Rambla. Dependencies and future updates are part of that decision. With the global `--host` option, commands run on the remote daemon host.
...
| Symptom                                                               | Check                                                                                                                                   |
| --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `This plugin was made for an older version of Rambla`                 | The directory has only an `index.ts` entry. Follow the [migration guide](/docs/plugins/migration).                                      |
