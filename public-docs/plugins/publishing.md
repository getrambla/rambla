...
description: Share a Rambla plugin on npm, through a private registry, or from a Git repository.
nav: Publishing
order: 45
category: Plugins
---
...
Publish your plugin so other people can install and use it in Rambla. Start with a working
...
```bash
npm pkg set name=@acme/rambla-review version=1.0.0
...
```bash
rambla plugin install npm:@acme/rambla-review@1.0.0
```

Users can also paste `npm:@acme/rambla-review` into **Settings → Plugins → Plugin source**.
...
- Add other runtime libraries with `npm install <package>`. Rambla installs their dependencies too.
...
Rambla compiles TypeScript. An ordinary plugin needs no separate build before publication.
...
Rambla skips npm lifecycle scripts during installation, including dependency scripts. For example,
...
To install the plugin, configure npm **on the daemon host, as the user running Rambla**:
...
Rambla uses the host's npm registry settings and credentials for installation and updates.
...
```bash
rambla plugin install git:https://git.example.com/acme/rambla-review.git
```

If your plugin has runtime npm dependencies, commit `package-lock.json` and add a preparation
command to `rambla-plugin.json`:
...
Plugins that only use Rambla's host libraries need no preparation command.
