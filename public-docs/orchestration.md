...
Rambla lets your coding agents coordinate other agents, split work across providers and machines, and keep tasks moving automatically.
...
- **Work across machines:** use the [CLI](/docs/cli#connecting-to-a-remote-daemon) to launch and manage agents on another reachable Rambla host.
...
Use built-in Rambla tools or the CLI. Both let an agent launch and coordinate workers.

### Built-in Rambla tools (MCP)

Enable Rambla tools so agents running inside Rambla can manage agents and workspaces on their host directly.

1. Open **Settings → your host → Agents**.
2. Turn on **Enable Rambla tools**. Tool injection is off by default.
3. Start a new agent, or reload an existing agent so it receives the tools.
4. Ask:

> Use Rambla to launch a second agent to review this branch. Have it report potential bugs without changing files, then summarize its findings.
