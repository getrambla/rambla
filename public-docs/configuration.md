...
You can change the home directory by setting `RAMBLA_HOME` or selecting `--home` on a CLI command.
...
```bash
rambla daemon config get daemon.listen --home ~/rambla-test
rambla daemon config set daemon.listen 127.0.0.1:6800 --home ~/rambla-test
rambla daemon config unset features.webUi.enabled --home ~/rambla-test
```
...
`RAMBLA_RELAY_ENABLED=true|false` overrides the file for a foreground deployment. Managed `start` uses the file. End and relaunch a deployment to remove its override before changing relay from the app or `rambla daemon pair --relay`.
