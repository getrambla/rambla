...
description: Serve the Rambla web app from your own daemon and reach it over your own LAN, VPN, reverse proxy, or tunnel.
nav: Web UI
order: 6
...
Rambla's daemon can serve the browser web app itself, from the same address it already uses for the API. You don't need the hosted app at [app.rambla.sh](https://app.rambla.sh): point a browser at your own daemon and you get the full UI, connected to your own agents, on infrastructure you control.
...
For the remote/relay path (driving a daemon through the Rambla relay rather than a reverse proxy), the relay has its own public-vs-internal TLS settings, see [Security](/docs/security).
