...
organization. Set the Hub origin in `RAMBLA_HUB_URL` below, for example
...
```bash
curl --fail-with-body -sS -X POST "$RAMBLA_HUB_URL/api/v1/configurations/install" \
...
```bash
curl --fail-with-body -sS -X POST "$RAMBLA_HUB_URL/api/v1/manual-runs" \
  -H "Authorization: Bearer $RAMBLA_HUB_API_KEY" \
  -H "Content-Type: application/json" \
  --data '{
    "projectSlug": "my-project",
...
curl --fail-with-body -sS -X POST \
  "$RAMBLA_HUB_URL/api/v1/daemons/enrollment-tokens" \
  -H "Authorization: Bearer $RAMBLA_HUB_API_KEY"
```
