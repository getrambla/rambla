...
Read the **rambla** skill. Call `list_profiles` before choosing the advisor. Do not create the advisor until you have read the configured profiles and their `notes`.
...
Materialize the selected profile into `create_agent` as described by the **rambla** skill. If no profile fits, use Rambla's provider discovery fallback.
...
- `/rambla-advisor /unslop` → advisor runs `/unslop` on the current diff.
- `/rambla-advisor /unslop-risk` → advisor does an unslop-risk review.
- `/rambla-advisor $diagnose this build failure` → advisor invokes `/diagnose`.
