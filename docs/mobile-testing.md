...
Metro, and device through the `RAMBLA_COMPOSER_KEYBOARD_*` variables in
...
- `flows/launch.yaml` — handles dev launcher, dismisses dev menu, asserts "Welcome to Rambla"
...
# Start voice mode in an existing composer, then background Rambla with Home.
adb emu gsm call 5551234
# Foreground Rambla while the call is still ringing.
```

Expected result: Rambla does not throw `RuntimeException: Audio focus request failed`; native audio reports an interruption and voice mode stops or pauses coherently.

### Releasing the audio session when idle

Rambla must not hold the OS audio session once it is neither capturing nor playing, or the user's
...
open Rambla, use dictation once, stop, and confirm the music resumes at full quality; then
...
```bash
cd packages/app/ios
xcodebuild -workspace RamblaDebug.xcworkspace -scheme ExpoTwoWayAudio \
