...
Provider message IDs are not guaranteed for every displayed item. Rambla-generated system errors are one example. Rendered item indices are not durable either because pagination and projection can merge source rows.
...
the Rambla timeline, so one visible response can span several canonical turns without a user message
