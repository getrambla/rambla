...
Desktop dev launches its desktop-managed daemon with `RAMBLA_NODE_ENV=development`,
...
`EXPO_PUBLIC_RAMBLA_DEV_BUILD_LABEL`. The expanded desktop sidebar shows it in
...
```bash
RAMBLA_HOME=~/.rambla-blue npm run dev          # explicit home
RAMBLA_DEV_SEED_HOME=/path/to/home npm run dev # seed from a different source home
RAMBLA_DEV_RESET_HOME=1 npm run dev            # clear and reseed the derived worktree home
```
...
- **An iOS Simulator runtime with at least one iPhone device type**. Recent Xcode versions may not bundle a runtime — add one via Xcode → Settings → Components (older Xcode: "Platforms"). The service targets `iPhone 16 Pro` by default (override with `RAMBLA_IOS_DEVICE_TYPE`) and falls back to any iPhone; it fails with `No iPhone simulator device type is installed` when none exist.
...
`RAMBLA_ELECTRON_REMOTE_DEBUGGING_PORT` when a QA workflow requires a validated,
fixed port.
...
`RAMBLA_ELECTRON_REMOTE_DEBUGGING_PORT` environment names as desktop dev. Set an
...
`RAMBLA_PORT=57928` alone starts Metro on 57928 but Electron still loads 8081.
...
Captured samples are exposed on `globalThis.__RAMBLA_RENDER_PROFILE__`. Call
`globalThis.__RAMBLA_RESET_RENDER_PROFILE__?.()` after warm-up and before the
...
reason counts are exposed on `globalThis.__RAMBLA_RENDER_PROFILE_REASONS__`.
...
```bash
RAMBLA_PROFILE_APP_URL=http://localhost:19010 # Expo web URL
RAMBLA_PROFILE_SWITCH_COUNT=1                # number of agent/terminal switch pairs
RAMBLA_PROFILE_SWITCH_WAIT_MS=250            # delay after each click
RAMBLA_PROFILE_IDLE_WAIT_MS=3000             # idle baseline before switching
RAMBLA_PROFILE_DUMP_COMMITS=1                # include per-commit profiler samples
```
...
React commits on the same browser clock. Set `RAMBLA_PROFILE_WORKSPACE_DIGITS`,
`RAMBLA_PROFILE_WORKSPACE_LRU_SIZE`, or `RAMBLA_PROFILE_WARM_QUIET_MS` to change the shape. Set
`RAMBLA_PROFILE_DUMP_COMMITS=1` to include the nested component breakdown for every commit.
Set `RAMBLA_PROFILE_RETAINED_REPEATS=5` to repeat the retained-LRU burst for a less noisy sample.
Set `RAMBLA_PROFILE_CPU_PATH=/tmp/workspace-switch.cpuprofile` to run a separate retained-LRU
capture after the latency scenarios and write a raw CDP CPU profile without contaminating their
timings.
Set `RAMBLA_PROFILE_TRACE_PATH=/tmp/workspace-switch.trace.json` to run another separate retained-LRU
capture and write a Chromium Performance trace with User Timing marks and screenshots. Open the
trace in the Chrome DevTools Performance panel. Set `RAMBLA_PROFILE_TRACE_INVALIDATIONS=1` only for a
focused invalidation capture; React Native's generated stacks make that mode highly intrusive.
Set `RAMBLA_PROFILE_TRACE_FOCUS=1` to include focus targets, durations, and JavaScript call stacks in
...
`RAMBLA_PROFILE_CPU_PATH=/tmp/explorer-toggle.cpuprofile` for separate Chromium captures. Override
the app URL, daemon port, workspace, or server with the corresponding `RAMBLA_PROFILE_*` variables.
...
Playwright dispatch lateness. Set `RAMBLA_PROFILE_TYPING_KEYS`, `RAMBLA_PROFILE_TYPING_CADENCE_MS`,
`RAMBLA_PROFILE_TYPING_REPEATS`, or `RAMBLA_PROFILE_WORKSPACE_DIGIT` to change the scenario. Optional
`RAMBLA_PROFILE_CPU_PATH` and `RAMBLA_PROFILE_TRACE_PATH` captures run separately after the latency
measurements so profiling overhead does not contaminate them.

Set `RAMBLA_PROFILE_TYPING_SCENARIO=height-growth` to alternate `Shift+Enter` and printable input.
...
RAMBLA_DESKTOP_WINDOW_CONTROLS=linux npm run dev:desktop
```
...
Check `$RAMBLA_HOME/daemon.log` for daemon logs. The default level is `info`; set
`RAMBLA_LOG_LEVEL=trace` before launching the daemon when you need full provider,
...
`$RAMBLA_HOME/config.json` win first. Without persisted config, the optional
`RAMBLA_LOG_ROTATE_SIZE` and `RAMBLA_LOG_ROTATE_COUNT` env vars override the
...
the daemon-global Git process limits in `$RAMBLA_HOME/config.json`:
...
```json
{
  "scripts": {
    "web": {
      "type": "service",
      "command": "PORT=$RAMBLA_PORT npm run dev:web"
    }
  }
}
```
...
Set `RAMBLA_HOST` to use the same target across invocations. An explicit
selector overrides both environment selectors. With both `RAMBLA_HOME` and `RAMBLA_HOST` set, pass an explicit selector. See [CLI target selection](../public-docs/cli.md#select-one-daemon).
...
```bash
find $RAMBLA_HOME/agents -name "{agent-id}.json"
```

Find by content:

```bash
rg -l "some title text" $RAMBLA_HOME/agents/
```
