...
Rambla surfaces terminal activity as a tab indicator (the same "running" dot used by agents).
...
The plugin translates both event contracts into the existing Rambla hook events. OpenCode 2 disposes its event subscription when the plugin unloads.
...
When enabled, Rambla installs provider hooks globally:
