...
> **Trust every plugin you add.** `rambla plugin add` and `rambla plugin install` mean “I trust this codebase.” Plugins are unsandboxed: server code and preparation commands run with the daemon user's access on the daemon host, and client contributions run inside Rambla. The repository's dependencies and future updates are part of that trust decision. With `--host`, preparation runs on that remote daemon host.
...
The directory contains a manifest declaring identity and Rambla requirements, one optional entry per runtime, runtime-owned
...
Declare the supported Rambla range and keep it current when adopting newer APIs. See the
...
Removing a Git source deletes Rambla's managed checkout.
...
shell command strings are rejected. Rambla starts the executable directly, without a shell, from the
plugin directory in the staged checkout. It never detects lockfiles or chooses a package manager.

On install and every update, Rambla resolves the exact Git revision and manifest, runs the declared
commands, then validates, compiles, and activates the candidate. It logs each argv command and its
output in the daemon log. If a command fails, the error includes its output, Rambla discards the
...
The contribution function must return cleanup. Server cleanup may be async; Rambla waits for it when
the plugin is reloaded, disabled, removed, disconnected, or shut down. Cleanup is for resources
created by plugin code. Rambla removes registered contributions, unmounts surfaces, clears query
...
Rambla owns the route, screen header, Lucide icon validation, close action, theme DTO, layout facts,
...
Use plugin RPC for plugin-specific backend behavior that is not a normal Rambla operation.
...
When the same plugin contribution exists on multiple hosts, Rambla shows it once in the sidebar and
...
Command Center callbacks use the selected host's existing `RamblaApi` for normal Rambla operations.
...
Provider settings are toggle/select data that Rambla renders in the composer. Keep private options in
...
Request observation with `client.rambla.agents.list({ subscribe: {} })` and consume the returned
...
Rambla memoizes by source-item reference and phase and derives replacement IDs from source identity, so
...
replaces it, and an empty array removes it. Output `data` must be JSON-compatible. Rambla adds the
...
Rambla owns the autocomplete row, input clearing, and error toast. It never sends the command text to
...
Register a declarative attachment source backed by a plugin RPC. Rambla owns the attachment menu,
...
`addTheme` takes a small light or dark palette and a display name. Rambla expands it through the
