...
// The startup splash sweeps a themed gradient through a CSS mask, so the web
// needs the mark as a shape-only data URL rather than a tinted component. It
// stays inline because the splash is the first paint and a fetched mask would
// flash an unmasked rectangle.
const maskSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${art.vbX} ${art.vbY} ${art.vbW} ${art.vbH}" fill="none">${art.recolor("#000000")}</svg>`;
writeFileSync(
  out("packages/app/src/components/icons/rambla-logo-mask.ts"),
  `// Generated by fork/brand/generate.mjs from fork/brand/rambla-logo.svg. Do not edit by hand.
export const RAMBLA_LOGO_MASK_URL = ${JSON.stringify(`url("data:image/svg+xml,${encodeURIComponent(maskSvg)}")`)};
`,
);
