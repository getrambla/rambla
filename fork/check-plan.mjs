...
const statusIdx = bodyAfterTitle.findIndex((l) => l.startsWith("Status: "));
const firstHeadingIdx = bodyAfterTitle.findIndex((l) => l.startsWith("## "));
if (statusIdx === -1 || (firstHeadingIdx !== -1 && statusIdx > firstHeadingIdx)) {
  problems.push(
    `${file}  missing status line — "Status: <value>" must be the first line under the title`,
  );
} else {
...
if (acIdx !== -1) {
  ...
  const end = next === -1 ? lines.length : lines.indexOf(`## ${headings[next]}`, start + 1);
  const items = lines.slice(start + 1, end).filter((l) => /^\d+\. /.test(l.trim()));
  if (items.length === 0) {
    problems.push(
      `${file}  Acceptance criteria has no numbered items — plans propose criteria as a numbered list`,
    );
  }
}
