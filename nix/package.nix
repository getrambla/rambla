...
buildNpmPackage rec {
  ...
  installPhase = ''
    ...
    mkdir -p $out/lib/rambla
    node scripts/trace-daemon.mjs > daemon-files.txt

    while IFS= read -r path; do
      [ -z "$path" ] && continue
      mkdir -p "$out/lib/rambla/$(dirname "$path")"
      cp -a "$path" "$out/lib/rambla/$path"
    done < daemon-files.txt

    # Root package.json lets node resolve the workspace layout when the
    # CLI/server bin starts from $out.
    cp package.json $out/lib/rambla/

    # Web UI Assets
    cp -r packages/server/dist/server/web-ui $out/lib/rambla/packages/server/dist/server/
    ...
    makeWrapper ${nodejs}/bin/node $out/bin/rambla-server \
      --add-flags "$out/lib/rambla/packages/server/dist/scripts/supervisor-entrypoint.js" \
      --set RAMBLA_NODE_ENV production

    # Create wrapper for the CLI
    makeWrapper ${nodejs}/bin/node $out/bin/rambla \
      --add-flags "$out/lib/rambla/packages/cli/dist/index.js" \
      --set NODE_PATH "$out/lib/rambla/node_modules"
