...
- **This codebase is upstream's (getpaseo/paseo) unless `PATCHES.md` records the change as ours or ours differs from upstream/main.** Never attribute upstream's code to the author of this repo, not "your code", not "your fork already has it", not "you wrote". Most bugs are inherited, and we are free to overwrite and replace anything we want. Check before attributing: `git log upstream-rebrand -- <path>` (any history = upstream's) or `git ls-tree upstream-rebrand <path>` (empty = ours). Always `upstream-rebrand`, never `upstream/main` — main is unrebranded, so a path we renamed does not exist there and the check silently answers "ours" for upstream's own files. Default to saying "upstream code"; say "ours" only after running one of those.
...
- **Before fixing or modifying any file that exists upstream, diff it against `upstream-rebrand` first** (`git diff HEAD upstream-rebrand -- <path>`). Never diff against `upstream/main`: it is unrebranded, so every line mentioning the brand shows as a difference and the real changes are buried. If upstream already fixed it, port their fix verbatim — do not invent a parallel solution. Invented fixes create merge conflicts and drift; this happened with the terminal SDK e2e EBUSY fix (2026-09-21), where a proposed helper file would have conflicted with upstream's one-line `maxRetries` fix.
