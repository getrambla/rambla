...
- **Never pass `--foreground` (or any removed launch flag) to `rambla daemon run/start`, especially in the justfile's systemd unit.** Upstream removed those flags; the CLI hard-errors on them (`REMOVED_LAUNCH_FLAGS` in `packages/cli/src/commands/daemon/local-daemon.ts`) and the daemon fails to start under systemd. `rambla daemon run` is already foreground — do not "make it explicit". Same for `rambla daemon start`: it is already correct without flags.
